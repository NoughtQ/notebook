...
- **语言生成器**(language generator)：生成语法正确的字符串的设备
    - [正则表达式](2.md#regular-expressions)可看作一种语言生成器
...
- 称形如以下形式的任意序列为从 $w_n$ 到 $w_0$ 的 $G$ 的**推导**(derivation)
...
>注：教材中把 $A \rightarrow a$ 也列在违背 CNF 的短规则中，笔者认为是有问题的，因为这条规则明明符合 CNF 的定义，于是把它剔除了。
...
!!! note "结论"
    ...
    任何满足 CNF 形式的 CFG 推导出的字符串长度为 $n \ge 1$ 的字符串恰好需要 $2n - 1$ 步推导。
