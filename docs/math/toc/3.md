...
???+ question "题集"
    ...
    === "问题4"
        ...
        === "题目"
            ...
            设计一个 CFG，生成语言 $L = \{ uv : u, v \in \{0, 1\}* \text{ and there are exactly 2 different symbols between} v \text{ and } u^R \}$，要求最多 3 个非终结符且最多 12 条规则。
        ...
        === "解答"
            ...
            - $S$：起始状态，表示还需要产生 2 个不匹配
            - $A$：中间状态，表示还需要产生 1 个不匹配
            - $B$：收尾状态，表示还需要产生 0 个不匹配（剩下的部分就是回文了）
