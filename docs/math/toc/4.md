...
假定**纸带**(tape)有左端，但右边是无限延伸的。为避免读写头移出左端，规定**最左端**始终用特殊符号 $\triangleright$ 表示，并假设读写头遇到该符号后就立即向右移动。
...
???+ abstract "总结：语言的分类"
    ...
    >偷了 [xg 的图](https://note.tonycrane.cc/cs/tcs/toc/topic4/#_4)（逃x
...
!!! definition "定义"
    ...
    **随机访问图灵机**(random access Turing machine)是一个二元对 $M = (k, \Pi)$，其中 $k > 0$ 表示**寄存器**数量，**程序** $\Pi = (\pi_1, \pi_2, \dots, \pi_p)$ 是一个有限的指令序列，其中每条指令 $\pi_i$ 是来自上面表格中的其中一个类型。假设最后一条指令 $\pi_p$ 始终是一个 `halt` 指令（程序的也可以包含其他 `halt` 指令）。
...
??? example "原始递归函数的例子"
    ...
    - 整除商 $\begin{cases}\text{div}(0, n) = 0 \\ \text{div}(m + 1, n) = \begin{cases}\text{div}(m + 1, n) + 1 & \text{if equal}(\text{rem}(m, n), \text{pred}(n)) \\ \text{div}(m, n) & \text{otherwise}\end{cases}\end{cases}$

我们可以用取值仅为 0 和 1 的原始递归函数来构建**原始递归谓词**(primitive recursive predicate)，比如 $\text{greater-than}(m, n)$。由此我们可以得到**按情况定义的函数**(function defined by cases)：
