...
!!! warning "注意"
    ...
    本章笔记的证明是 Gemini 3 Pro 基于教材内容整理得到的（笔者稍微整理了一下排版），可能存在错误。若读者发现错误，可以在评论区中指出🙇‍♂️。

## Definition

图灵机的设计是为了同时满足以下三条标准：

- 应当是自动机，也就是说构造和功能应该与之前研究过的设备具有相同的一般精神
- 描述、形式定义和推理应当尽可能简单
- 执行的计算应尽可能一般

**图灵机**(Turing machine)由一个有限状态控制单元、一条纸带和一个能在纸带上读写的头构成。
...
假定**纸带**(tape)有一个左端，但右边是无限延伸的。为避免读写头移到左端外面，假设**最左端**始终用特殊符号 $\triangleright$ 表示，并假设读写头遇到该符号后就立即向右移动。
...
1. $M$ 仅重写符号，并不移动读写头
2. $M$ 将读写头左移一格；若正从空纸带中移出，那么当前扫描到的空符号就会从配置中消失
3. $M$ 将读写头右移一格；若正移入空纸带，空符号将作为新的扫描符号被加入到配置中
...
!!! definition "定义"
    ...
    令 $M = (K, \Sigma, \delta, s, H)$ 为一台图灵机，且 $H = \{y, n\}$ 包含两个不同的停机状态（分别表示「是」和「否」）。任何包含状态 $y$ 的停机配置被称为**接受配置**(accpeting configuration)，而任何包含状态 $n$ 的停机配置被称为**拒绝配置**(rejecting configuration)。
...
换句话说，当对于任何输入 $w$，总能进入一个对输入正确响应的停机状态：$y \text{ if } w \in L, n \text{ if } w \notin L$，那么称图灵机接受了语言 $L$。需要注意的是，无法保证当输入包含空符号或左端符号时机器会发生什么。
...
和其他语言识别器不同的是，即便图灵机有两个停机状态 $y, n$，总是有可以规避这两个答案的选项，这会使得停机失败。
...
!!! definition "定义"
    ...
    令 $f$ 为从 $\Sigma_0^*$ 到 $\Sigma_0^*$ 的函数。当且仅当 $\forall\ w \in \Sigma_0^*$，$M(w) = f(w)$，称 $M$ **计算**(computes)了函数 $f$。像这种能被图灵机计算出来的函数被认为是**递归的**(recursive)。
...
我们不关心具体到达的停机配置，只要能到达某个停机配置即可。若 $w \in \Sigma_0^* - L$，那么 $M$ 就不能进入停机状态。

接下来扩展图灵机的函数记号：用 $M(w) = \nearrow$ 表示 $M$ 未能在 $w$ 上停机。这样我们就能重写半判定的定义：$\forall\ w \in \Sigma_0^*, M(w) = \nearrow$ 当且仅当 $w \notin L$。

看起来图灵机的半判定定义是对确定型有限自动机的可接受性的直接扩展，但有一个很明显的区别是：有限自动机始终会停机，而半判定语言 $L$ 的图灵机无法始终能够判断字符串 $w$ 是否属于 $L$，因为如果 $w \notin L$，我们永远无法知道要等多久才能得到答案。

!!! theorem "定理"

    若某种语言是递归的，那它也是递归可枚举的。
...
!!! theorem "定理"
    ...
    若语言 $L$ 是递归的，那么它的补 $\overline{L}$ 也是递归的。
...
到目前为止，我们介绍的图灵机虽然强大，但是慢而笨拙。为更好地理解它的强大之处，接下来将从不同方向扩展图灵机模型。但无论哪种情况下，这些看似新改进的模型最终都可以用标准图灵机模型来模拟，这也证实了图灵机确实是终极的计算设备。
...
!!! definition "定义"
    ...
    **随机访问图灵机**是一个二元对 $M = (k, \Pi)$，其中 $k > 0$ 表示**寄存器**数量，**程序** $\Pi = (\pi_1, \pi_2, \dots, \pi_p)$ 是一个有限的指令序列，其中每条指令 $\pi_i$ 是来自上面表格中的其中一个类型。假设最后一条指令 $\pi_p$ 始终是一个 `halt` 指令（程序的也可以包含其他 `halt` 指令）。
...
??? example "例子"
    ...
    来看如何用随机访问寄存器计算有限二元关系的自反传递闭包，也就是说给定有向图 $R \subseteq A \times A$，其中 $A = \{a_0, \dots, a_{n-1}\}$，我们希望计算 $R^*$。
...
!!! theorem "定理"
    ...
    任何由随机访问图灵机判定或半判定的语言，以及任何由随机访问诱导机可计算的功能，都可以分别由标准图灵机判定、半判定和计算。

    此外，如果机器在某个输入上停止，那么标准图灵机在该输入上所采取的步骤数由随机访问图灵机在该输入上所采取的步骤数的**多项式**所界定。

    ??? proof "证明"

        通过构造一个拥有 $k+3$ 条磁带的标准图灵机 $M'$ 来模拟具有 $k$ 个寄存器的随机存取图灵机。
...
??? example "例子"
    ...
    对上述机器加以改进，可以用来判定语言 $C$（一个放有所有合数二进制表示的集合）。
...
!!! theorem "定理"
    ...
    如果一个非确定性图灵机 $M$ 半判定或判定了一个语言，或计算了一个函数，那么存在一个标准图灵机 $M'$ 半判定或判定相同的语言，或计算相同的函数。
    ...
    ??? proof "证明"
        ...
        2. 构造确定性图灵机：
            - 为了消除非确定性，我们定义一个确定性图灵机 $M_d$；它有两条磁带：第一条放原始输入 $w$，第二条放一个由 $\{1, 2, \dots, r\}$ 构成的提示串(hints)（例如 $i_1 i_2 \dots i_n$）
...
图灵机及其扩展均属于**语言接受器**(language acceptors)。但我们知道还有另一类叫做**语言生成器**(language generators)的重要的计算设备，比如比如[正则表达式](2.md#regular-expressions)和[上下文无关文法](3.md#context-free-grammars)（CFG）。接下来就要介绍 CFG 一种推广形式：**文法**(grammar)（或**无限制文法**(unrestricted grammar)），并且说明由它产生的语言就是递归可枚举语言。
...
!!! definition "定义"
    ...
    **文法**（无限制文法/重写系统(rewriting system)是一个四元组 $G = (V, \Sigma, R, S)$，其中：
    ...
    - $R$：**规则**集合，是 $(V^*(V - \Sigma)V^*)^* \times V^*$ 的有限子集
...
!!! theorem "定理"
    ...
    ??? proof "证明"
        ...
        - **必要性**(only if)（文法 $\Rightarrow$ 递归可枚举）：给定一个文法 $G$，我们可以构造一个非确定性图灵机 $M$ 来**半判定**该语言。
            ...
            - 模拟过程：$M$ 在每一步非确定性地从 $G$ 的规则集 $R$ 中选择一条规则 $u \rightarrow$。它扫描第二条磁带，找到子串 $u$ 并将其替换为 $v$（需要适当移动后续字符以调整空间）
...
!!! theorem "定理"
    ...
    当且仅当函数 $f\ :\ \Sigma^* \mapsto \Sigma^*$ 是文法可计算的时候，该函数是递归的。 
...
若一个函数可以通过基本函数的组合、递归定义和可最小化函数的最小化操作获得，则称该函数为 **$\mu$-递归**。
