...
- 连续型：设随机变量$X$的密度函数为$f(x)$，若$\int_{-\infty}^{+\infty} |x|f(x) \text{d}x < +\infty$，则称该积分为$X$的**期望**（均值），记为$E(X) = \int_{-\infty}^{+\infty} xf(x) \text{d}x$
    ...
    - 若已知二维随机变量$(X, Y)$的联合密度函数$f(x, y)$，那么：
        - $E(X) = \int_{-\infty}^{+\infty} \int_{-\infty}^{+\infty} xf(x, y) \text{d}x \text{d}y, E(X) = \int_{-\infty}^{+\infty} \int_{-\infty}^{+\infty} yf(x, y) \text{d}x \text{d}y$
...
- $X$的**$k$阶中心矩**：$\upsilon_k = E[(X - E(X))^2]$
