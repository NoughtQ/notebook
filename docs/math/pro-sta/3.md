...
??? example "例题"
    ...
    === "答案"
        ...
        <div style="text-align: center">
            ...
            <img src="images/C3/5.png" width="45%">
        </div>
...
- 当且仅当对任意实数$x, y$，有
    ...
    成立，即$F(x, y) = F_X(x) \cdot F_Y(y)$，$X, Y$（联合分布函数 = 边际分布函数的乘积）相互**独立**
...
- $(X, Y)$为二维连续型随机变量，设联合密度函数为$f(x, y)$，则$Z$的分布函数为：
    ...
    & = \iint\limits_{x + y \le z} f(x, y)\text{d}x\text{d}y = \int_{-\infty}^{+\infty} \text{d}x  \int_{-\infty}^{+\infty} f(x, y) \text{d}y \notag
    \end{align}
    $$
    ...
    - 特别地，当$X, Y$相互独立时，上式可以写成如下形式（称为**卷积公式**）：
