...
$$
\bm{x^{(k)}} \approx \lambda_1^k \beta_1 \bm{v_1}, \bm{x^{(k-1)}} \approx \lambda_1^{k-1} \beta_1 \bm{v_1} \Rightarrow \dfrac{(\bm{x^{(k)}})_i}{(\bm{x^{(k-1)}})_i} \approx \lambda_1
$$
...
要想迭代计算 $\bm{x^{(k+1)}} = A^{-1} \bm{x^{(k)}}$，就得在每次迭代步骤中求解线性方程组 $A \bm{x^{(k+1)}} = \bm{x^{(k)}}$，其中 $A$ 需要被分解(factorized)。
...
??? example "例题"
    ...
    === "解答"
        ...
        A、B 为幂法，C、D 为逆幂法；且 $\lambda_2$ 更接近真值，所以 D 的收敛速度最快。
