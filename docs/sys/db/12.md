...
我们先来看如何为查询选择最优的连接顺序。对于连接运算 $r_1 \bowtie r_2 \bowtie \dots \bowtie r_n$，总共有 $\dfrac{(2(n-1))!}{(n-1)!}$ 种不同的连接顺序，不难看出这个量会变得特别大，因此想要枚举所有连接顺序是不切实际的。但是通过一些观察，可以发现这其中有不少顺序是可以完全被忽略的。我们接下来会采用一种[**动态规划**](../../algo/ads/8.md)(dynamic-programming)算法来寻找最优的连接顺序（其中集合 $S$ 包含上述 $n$ 个关系的集合）：
