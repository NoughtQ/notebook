...
考虑包含 $n$ 个进程 $\{P_0, P_1, \dots, P_{n-1}\}$ 的系统，每个进程有一个称为**临界区**(critical section)的代码段。在此区域内，进程可能正在访问并更新与至少一个其他进程共享的数据。

而**临界区问题**(critical-section problem)旨在设计一种协议，使进程能够同步它们的活动，保证不会出现两个进程同时执行各自临界区的内容。因此每个进程在进入临界区前必须要发起请求，其中
...
- **有限等待**(bounded waiting)：存在一个限制，即在某个进程请求进入其临界区之后、该请求被批准之前，允许其他进程进入各自临界区的次数上限
...
- 对于单核环境，临界区问题可通过在修改共享数据时禁用中断来解决，此时可确保当前执行的指令序列不会被抢占，因而不会出现意料之外的修改。
...
???+ info "注"
    ...
    尽管该方法**无法确保**在这些架构上正确工作，但这里仍然要先介绍这一方法，这是因为它提供了求解临界区问题的很好的算法描述，并阐述了设计满足上述三个要求的软件所涉及的复杂性。
...
- **弱有序**(weakly orderly)：对某个处理器的内存修改可能无法立马被其他处理器看到

计算机系统提供了一种能让处理器对内存的任何修改强迫传播给所有其他处理器的指令，从而确保所有的内存修改对正在其他处理器上运行的线程都是可见的。这种指令称为**内存屏障**(memory barrier / fence)。执行内存屏障指令时，系统确保所有的加载和存储在之后的加载和存储执行前执行完毕。所以即便存在指令重排，内存屏障指令也能确保在未来的加载或存储操作执行之前，存储操作已在内存中完成，并对其他处理器可见。
...
值得注意的是，内存屏障指令是低级操作，通常只能被内核开发者使用，用于编写互斥相关的代码。
...
- `test_and_set()`（测试并设置）
    ...
    - 如果机器支持这个指令，那就可以通过声明一个 `#!java boolean` 变量 `lock`（初始化为 `#!c false`）来实现互斥，此时进程 $P_i$ 的结构如下所示：
    ...
    - 但此时算法仍未满足有限等待要求，所以下面给出更完善的代码，能够满足所有解决临界区问题的要求
        ...
        ??? proof "证明"
            ...
            === "互斥"
                ...
                - 变量 `waiting[i]` 仅当另一个进程离开其临界区时才会变为 `#!false`，且只有一个进程的 `waiting[i]` 被设为假，从而维持了互斥要求
...
上述基于硬件的解决方案不仅复杂，而且不方便为应用程序员使用。因此 OS 开发者提供了更高级的软件工具来解决临界区问题，其中最简单的工具就是**互斥锁**(mutex locks)。我们将使用互斥锁保护临界区，从而阻止竞态条件发生。具体来说，进程在进入临界区时必须要获取锁，当退出临界区时要释放锁（对应的操作为 `acquire()` 和 `release()`，如下所示）。
...
调用这两个函数时必须以原子形式执行，因此互斥锁可用前面介绍的 CAS 操作实现。但这种实现的一个缺点是会导致**繁忙等待**(busy waiting)：当一个进程处于其临界区时，任何其他试图进入自身临界区的进程必须在调用 `acquire()` 时持续循环等待。这显然会浪费 CPU 时间。
...
目前对 `wait()` 和 `signal()` 的定义仍然会导致繁忙等待的问题出现，所以需要修改它们的定义为：当进程执行 `wait()` 操作时且发现信号量非正，那进程必须等待。

- 但进程并非进入繁忙等待，而是把自己**挂起**(suspend)，即把进程放入与信号量关联的等待队列中，并且将该进程的状态切换至等待状态。随后控制权转交给 CPU 调度器，它会选择另外的进程执行。
- 当其他进程执行 `signal()` 操作后，先前被挂起的进程就应该要**重启**了，此时该进程从等待状态变为就绪状态，并进入就绪队列中。
...
- 值得注意的是，上述改进定义仍然没能消除繁忙等待的问题，只是将这一问题从进入区移动到临界区内，但好在临界区内的繁忙等待频率会少很多
