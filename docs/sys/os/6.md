...
??? question "例题"
    ...
    === "题目"
        ...
        Here are four processes P1, P2, P3 and P4, each of which prints the process name of itself. They must run in the order below:

        - P2 can run only after P1 finishes.
        - P3 can run only after both P1 and P2 finish.
        - P4 can run only after P3 finishes.

        Use three semaphores `P1_done`, `P2_done` and `P3_done` to keep them in order. All initial values of them are `0`. Complete the code below, fill one sentence of `wait` or `signal` in any underlined blank line each.
    ...
    === "解答"
        ...
        >题目很简单（~~但不要像笔者那样把它们看作一段完整的程序~~，实际上每个注释下的代码段对应一个进程要执行的程序）
