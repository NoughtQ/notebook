...
??? abstract "核心知识"
    ...
    1. 规范树：消除 `ESEQ` -> 将 `CALL` 移到顶层 -> 消除 `SEQ`
    2. 基本块：`LABEL` 开头，`JUMP` / `CJUMP` 结尾
    3. 轨迹：考虑 `CJUMP` / `JUMP` 两种情况

语义分析阶段生成的 IR 树随后会被翻译成汇编语言或机器语言。虽然树语言的运算符经过精心选择，能够匹配大多数机器的能力，但它与机器语言并不完全对应，且会会干扰编译时的优化分析，比如：
...
- 表达式中的 `CALL` 节点也会导致同样的问题，比如当尝试将参数放入一组固定的形参寄存器时，其他 `CALL` 节点的参数表达式中的 `CALL` 节点会引发问题，例如：`CALL(f, [e1, CALL(g, [e2, ...])])`

解决方法是通过 3 个步骤对树做变换处理，将树状结构转换为容易被机器处理的**线性结构**：
...
由性质 1 得到：每棵规范树仅包含**一个语句节点**，即根节点；其他节点均为表达式节点
...
思路：将 `ESEQ` 在树中不断向上提升，直到其父节点是语句节点，从而可以转换为 `SEQ` 节点。
...
`s`, `e1` 可**交换**(commute)：如果 `s` 分配的临时变量和内存位置未被 `e1` 引用（且 `s` 和 `e1` 不同时执行外部 I/O）。
...
树语言允许将 `CALL` 节点用作子表达式。然而，所有函数的返回值同一个专用的返回值寄存器 `TEMP(RV)` 中，所以后面的调用产生的返回值会覆盖前面的调用。

```
BINOP(PLUS, CALL(...), CALL(...))
```

解决方法：将每个调用的返回值分配到**临时寄存器**中，即把 `CALL(fun, args)` 改写为：
...
- 一旦整个函数体 `s0` 被处理完毕，结果会得到一个树状结构 `s0'`，其中所有的 `SEQ` 节点都靠近顶部：`SEQ(SEQ(SEQ(..., sx), sy), sz)`
- 线性化函数重复应用此规则：`SEQ(SEQ(a, b), c) = SEQ(a, SEQ(b, c))`
...
??? example "例子"
    ...
    === "题目"
        ...
        >这里提到的 rules 就是前面讲过的步骤。

        1. `MOVE(MEM(ESEQ(SEQ(CJUMP(LT, TEMP#sub[i] , CONST#sub[0], L#sub[out], L#sub[ok]), LABEL#sub[ok]), TEMP#sub[i])), CONST#sub[1])`
        2. `MOVE(MEM(MEM(NAME#sub[a])), MEM(CALL(TEMP f, [])))`
        3. `BINOP(PLUS, CALL(NAME#sub[f], [TEMP#sub[x]]), CALL(NAME#sub[g], [ESEQ(MOVE(TEMP#sub[x], CONST#sub[0]), TEMP#sub[x])]))`
    ...
    === "解答"
        ...
        === "1"
            ...
            <div style="text-align: center">
                <img src="images/C8/17.png" width=80%>
            </div>

            规范树：

            <div style="text-align: center">
                <img src="images/C8/18.png" width=80%>
            </div>
        ...
        === "2"
            ...
            <div style="text-align: center">
                <img src="images/C8/19.png" width=30%>
            </div>

            规范树：

            <div style="text-align: center">
                <img src="images/C8/20.png" width=80%>
            </div>
        ...
        === "3"
            ...
            <div style="text-align: center">
                <img src="images/C8/21.png" width=70%>
            </div>

            规范树：

            <div style="text-align: center">
                <img src="images/C8/22.png" width=90%>
            </div>
...
在确定程序中跳转的目标位置时，我们就是在分析程序的控制流。**控制流**(control flow)是指程序中指令的执行顺序，不考虑寄存器和内存中的数据值，以及算术计算。在分析程序的控制流时，任何非跳转指令的行为都完全无关紧要，所以我们将任何**非分支指令**序列合并成一个**基本块**，并分析基本块之间的控制流。
...
???+ note "技巧"
    ...
    如果题目给的是像上面例子那样的代码，可以按以下规则快速划分基本块：

    - `goto` 跳到的语句为新的基本块的**开始**
    - `if` / `goto` 语句本身为基本块的**末尾**
...
$$
\begin{aligned}
&\text{Put all the blocks of the program into a list } Q.\\
&\mathbf{while}\ Q\ \text{is not empty}\\
&\qquad \text{Start a new (empty) trace, call it } T.\\
&\qquad \text{Remove the head element } b\ \text{from } Q.\\
&\qquad \mathbf{while}\ b\ \text{is not marked}\\
&\qquad\qquad \text{Mark } b;\ \text{append } b\ \text{to the end of the current trace } T.\\
&\qquad\qquad \text{Examine the successors of } b\ \text{(the blocks to which } b\ \text{branches);}\\
&\qquad\qquad \mathbf{if}\ \text{there is any unmarked successor } c\\
&\qquad\qquad\qquad b \leftarrow c\\
&\qquad \textit{(All the successors of }\ b\ \textit{are marked.)}\\
&\qquad \text{End the current trace } T.
\end{aligned}
$$
...
- 任何紧跟 `false` 标签的 `CJUMP`：保留原样（这种情况会有很多）
- 任何紧跟 `true` 标签的 `CJUMP`：交换 `true` 标签和 `false` 标签，并取反条件
- 任何 `CJUMP(cond, a, b, lt, lf)` 后跟既不是 `true` 标签也不是 `false` 标签的情况：发明一个新的 `false` 标签 `lf'`，并将单个 `CJUMP` 语句重写为三个语句，以实现 `CJUMP` 紧跟 `false` 标签的条件
    ...
    === "例1"
        ...
        === "解答"
            ...
            **(c) 更好。**
            ...
            原因：它把循环体 `body` 放在 `test` 前面，让循环体执行完后可以**顺序落到测试块**：
            ...
            * **(b)** 更差，循环继续时既要跳到 `body`，循环体结束后还要跳回 `test`。
            * **(c)** 只在进入循环前需要一次：
        ...
        === "解答"
            ...
            === "8.7"
                ...
                - B9:
                    ...
                    ```
                    LABEL L15
                    MOVE(TEMP RV, TEMP m)  // 假设 RV 为返回值寄存器
                    ```

                <div style="text-align: center">
                    <img src="images/C8/23.png" width=70%>
                </div>

                因此一种可能的划分为：

                - Trace 1：B1 -> B2 -> B3 -> B4 -> B5
                - Trace 2：B6 -> B7 -> B8
                - Trace 3：B9