...
语义分析阶段中的一个重要步骤是维护**符号表**(symbol tables)（**环境**(environments)）。它将标识符映射到其**类型**(types)和**位置**(locations)上。具体来说，环境是指由一些用 $\mapsto$ 表示的**绑定**(bindings)构成的集合，例如 $\sigma_0 = \{ g \mapsto \text{string}, a \mapsto \text{int} \}$。
...
???+ example "例题"
    ...
    === "题目"
        ...
        1. show new bindings in venv (value environment) and tenv (type environment) in:
            1. global scope
            2. entering parameter scope of function `f`
            3. inside let block within `f`（整理者注：指第 7 行结束、第 8 行未结束时）
        2. Which variable or symbol table entries do `x` and `a[0]` inside `g` refer to?
        3. Would program type-check perform successfully? explain in 1-2 sentences.
    ...
    === "解答"
        ...
        === "2"
            ...
            - `x` 引用的是函数 `f` 的形参 `x:int`，不是外层全局 `x`
            - `a[0]` 引用的是 `f` 内部 `let` 块中定义的局部变量 `a:arr`，下标访问结果类型为 `int`

        === "3"

            程序可以通过类型检查。因为：

            - `x:int`，`a[0]:int`，所以 `x+a[0]` 的结果为 `int`，符合 `g():int`
            - `g()` 的结果为 `int`，也符合 `f(x:int):int`
