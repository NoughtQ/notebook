...
- 创建一个新变量 j'
- 在每次赋值 i <- i + c 之后，添加赋值 j' <- j' + c * b，其中 c * b 是一个循环不变表达式，可在循环前序头部计算；如果 c 和 b 均为常量，则乘法可在编译时完成。
- 将 j 的（唯一）赋值替换为 j <- j'（执行死代码消除）
- 在循环前序头部的末尾，将 j' 初始化为 j' <- a + i · b
...
经过强度降低后，部分归纳变量在循环中完全**未被使用**，另一些**仅用于**与循环不变量的**比较**。而这些归纳变量可以被删除。
...
- 数组的边界通常形式为 0 <= i ∧ i < N
- 从循环 L 中消除边界检查的标准实际上相当复杂，具体请参考教科书
...
![](images/C18/17.png){ align=right width=20% }
...
- 在尾部(epilogue)中执行奇数次迭代 => 健壮

    <div style="text-align: center">
        <img src="images/C18/20.png" width=50%>
    </div>