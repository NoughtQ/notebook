...
在 CFG 中，循环是指一个包含头节点 h 的节点集合 S，并满足以下性质：
...
构建**循环嵌套树**(loop-nest tree)：

1. 计算流图 G 的支配节点
2. 构建支配树
3. 找出所有自然循环，从而得到所有循环头节点
4. 对于每个循环头 h，将 h 的所有自然循环合并成一个循环 loop[h]
5. 构建循环头（以及隐含的循环）树，使得如果在 loop[h1] 中包含 h2，则 h1 在树中位于 h2 之上
