...
从编译器编写者的角度来看，执行的目标程序在其自身的**逻辑地址空间**中运行，其中每个程序值都有一个位置。目标程序的运行时表示由**数据**和**程序**区域组成。内存的一种典型划分方式是：

- **代码区**(code)：可执行的目标代码
- **静态区**(static)：编译时即可确定大小的数据对象，例如全局常量、编译器生成的数据（如用于垃圾回收（GC））
- **栈区**(stack)：过程调用期间生成的称为**活动记录**(activation records)的数据结构
...
- 一些**局部变量**(local variables)位于此帧中，其他局部变量保存在机器寄存器中
- **返回地址**(return address)：控制应返回到（调用函数内的）位置；由CALL指令创建
...
- 现代机器拥有大量的**寄存器**(registers)（通常为 32 个），但许多不同的过程和函数需要使用寄存器，因此可能会产生冲突
- 假设：
    - `f` 正在使用寄存器 `r` 来保存一个局部变量，并调用过程 `g`
    - `g` 也使用 `r` 进行自己的计算
    - 在 `g` 使用 `r` 之前必须保存它（存储到栈帧中），并在 `g` 使用完毕后恢复它（从栈帧中取回）

- 如果调用者（本例中是 `f`）必须保存和恢复该寄存器，则 `r` 是**调用者保存的寄存器**(caller-saved register)
- 如果被调用者（本例中是 `g`）负责处理，则 `r` 是被**调用者保存的寄存器**(callee-saved register)
...
**非叶过程**(non-leaf procedure)需要将其写入堆栈（除非使用了跨过程的寄存器分配策略）；而**叶过程**则不需要这样做。
...
- 变量将通过**引用**(reference)传递，因此必须具有内存地址（例如 C 语言中的 `&`）
- 变量被当前过程内部**嵌套**的过程访问（对于跨过程的寄存器分配而言并非必需）
- 值的体积**过大**，无法存入单个寄存器
- 变量是**数组**，需要地址运算来提取其元素
...
- 每当调用函数 `f` 时，可以向其传递一个指向静态包含 `f` 的函数框架的指针，即**静态链接**(static link)
- **显示表**(display)：一个全局数组，在其第 `i` 个位置存放指向最近进入的、静态嵌套深度为 `i` 的过程框架的指针
...
`Tr_outermost` 用于返回最外层的层级，即 Tiger 主程序嵌套所在的层级。所有“库”函数均在此最外层声明，该层不包含框架或形式参数列表。