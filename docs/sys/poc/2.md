...
??? abstract "核心知识"
    ...
    - 基本概念：token、RE、DFA、NFA（计算理论都讲过了，这块应该算容易理解的）
    - RE -> NFA -> DFA -> Parser
    - DFA 最小化
...
下面是对应的转换规则（**汤普森构造法**(Thompson’s Construction)）：
...
- 如果在 states[d] 里有一个状态是（NFA 的）最终状态，则 state[d] 便是（DFA 的）最终状态
...
1. 假设所有**最终状态**都是等价的，所有**非最终状态**也都是等价的，这是**两个原始集合**
2. 移除非等价状态
    1. 分割一个集合，**当前仅当对于所有输入符号 a，trans[s,a]=trans[t,a]**，两个状态 s 和 t 应在同一集合；之后我们有一些新状态，需用新集合替换原始集合
    2. 考虑步骤 a 中的每个集合
3. 重复步骤 2，直到没有集合发生变化
4. **现在的集合是最小 DFA 中的状态**
    - 起始状态是包含以前起始状态的集合
    - 增边是自然的，因为集合内的状态通过某个输入符号会进入同一集合
