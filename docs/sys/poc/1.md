...
**编译器**(compiler)是一类将一种语言翻译成另一种语言的程序（由于计算的抽象性）。它是一种复杂程序，由数万乃至数百万行代码构成（比如我们熟知的 `gcc` 就有 7.5M 行代码）。
...
- **词法分析**(lexing / scanning / lexical analysis)：将程序字符流分解为**记号**(token)序列
...
- **语法分析**(parsing / syntactic Analysis)：将记号序列解析为语法结构
...
- **CFG**（控制流图(control flow graph)）：方便进行数据流分析，比如**活跃变量分析**(liveness analysis)等
...
| 章节 | 阶段 | 描述 |
| :--- | :--- | :--- |
| 2 | 词法分析(lex) | 将**源文件**分解成独立的词汇，即**记号**(tokens) |
...
| 6 | 栈帧布局(frame layout) | 以机器相关的方式将变量、函数参数等放入**激活记录（栈帧）**中 |
...
| 10 | 数据流分析(dataflow analysis) | **收集**关于程序变量**信息流的信息**，例如，**活跃变量分析**(liveness analysis)计算每个程序变量持有仍需使用的值（即变量是“活跃”的）的位置 |
...
- 构造器名称：在右侧指示（**括号内**）
- 右侧组件：在数据结构中表示
    - `CompoundStm` 在右侧有两个 $Stm$：$Stm -> Stm; Stm$
