...
由于要等很久后才能知道真正的目的地和虚假的目的地，于是我们列出一个**位置列表**，其中包含当前填充为 `NULL` 的位置，这些位置需要在已知 `t` 时填充为 `t`；另外再列出所有需要填充为 `f` 的位置。
...
要翻译 `v`，就得知道**帧指针**(frame pointer)和**字长**(word size)。我们在 `Frame` 模块中添加一个帧指针寄存器 `FP` 和一个常量，其值为机器的自然字长。
