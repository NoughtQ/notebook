...
一种方法是同时对所有类进行静态分析（使用**图着色算法**），为每个字段名找到某个**偏移量**，该偏移量可在包含该字段的每条记录中使用。
...
动态链接的问题是：全局着色只能在**链接时**(link-time)进行。但许多面向对象的系统只有在运行时加载新类的能力。链接时图着色对于允许**动态增量链接**的系统来说，会引发许多问题。


### Hashing

在每个**类描述符**中放置一个**哈希表**，将**字段**名称映射到偏移量，将**方法**名称映射到方法实例，这能很好地与独立编译和动态链接配合使用。

- `Ftab`：包含字段偏移量和方法实例
- `Ktab`：包含字段名称指针（用于冲突检测）
- 要获取对象 `c` 的字段 `x`，编译器：  
    - 从对象 `c` 的偏移量 0 处获取类描述符 `d`
    - 从地址偏移量 `d + Ktab + hash_x` 处获取字段名称 `f`
    - 测试 `f` 是否等于 `ptr_x`；如果是，则
        - 从 `d + Ftab + hash_x` 处获取字段偏移量 `k`
        - 从 `c + k` 处获取字段的内容
...
面向对象语言还需要能保护对象的字段，防止被其他对象的方法直接操作。

- **私有字段**：无法从对象外部声明的任何函数或方法中获取或更新的字段
- **私有方法**：无法从对象外部调用的方法
