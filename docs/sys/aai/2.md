...
??? abstract "总结"
    ...
    | C | 计数器(counter) | 用于循环，例如 `#!c for(int i=0; i<9; i++)` 中的 `i` |
...
可以看到，大多数种类的 x86 寄存器可以通过多种粒度(multiple granularities)访问。

!!! warning "注意"

    修改 32 位**部分寄存器**会将寄存器的其余部分（即第 32-63 位）设置为 0，但修改 8 位或 16 位部分寄存器不会影响寄存器的其余部分。
...
- **RFLAGS**：表示微处理器的**状态**，并控制其操作
    ...
    - 状态标志：
        ...
        - **P**（第 2 位）：如果结果的**最低字节**(least significant byte)包含偶数个 1，则设置奇偶标志；否则清除（偶校验 `PF` = 1；奇校验 `PF` = 0）。
...
- **系统管理模式**(system management mode, SMM)是一种专为系统控制活动设计的运行模式，通常对常规系统软件透明
- 电源管理是系统管理模式的一种常见应用
...
由 DOS 置于内存中的程序被加载至 **TPA**（暂驻程序区(transient program area)），位于驱动程序之上的首个可用内存区域。该区域由 DOS 维护的一个**空闲指针**指示。程序的加载过程由 DOS 内部的**程序加载器**(program loader)自动处理。
...
80286（24 位地址线）及后续型号因为支持更高位数的地址，但但一些商业软件的实现故意依赖这种回滚寻址的特性，所以 Intel 将这一特性留给用户决定是否开启。如果不开启就能访问更多的地址位。
...
- **LDTR**（局部描述符表寄存器）与 **TR**（任务寄存器）：指向 GDT 中的特殊系统描述符（例如 LDTR 寄存器作为 GDT 的选择器）
    - 要访问 LDT，LDTR 加载一个选择符；该选择符访问 GDT，并将 LDT 的地址、限制和访问权限加载到 LDTR 的缓存部分
    - TR 持有一个选择符，该选择符访问定义任务（一个过程或应用程序）的描述符，允许多任务系统以简单有序的方式在任务之间切换
...
- 第 0 位：**P**（存在(present)位）
    ...
    - 功能：
        - 实现虚拟内存：这是[**按需分页**](../os/10.md#demand-paging)(demand paging)和**页交换**(page swapping)的基础
...
- 每当遇到**页错误异常**时，CPU 会将**引发该错误的线性地址**保存到 CR2 寄存器中（和 RISC-V 中的 stval 很像）
