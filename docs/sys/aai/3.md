...
要想为微处理器进行高效的软件开发，需要完全熟悉每条指令所采用的**寻址方式**。本章将解释**栈内存**的操作原理，以便理解 `PUSH`、`POP` 指令，及其他栈相关操作。

- 地址大小(address size)：用于访问内存位置的地址宽度，规定了 CPU 能够用来定位数据或指令的地址范围
- 操作数大小(operand size)：指令所处理数据的宽度，决定了算术、逻辑或数据传输操作中处理的数据的大小

存在三种操作模式，其默认地址大小和操作数大小如下：
...
当处理器执行指令时，它在数据上执行特定功能。而**寻址模式**(addressing modes)就是用于指定操作数地址的技术，并且各种不同的寻址模式，包括：
...
其中**源**(source)操作数在右，**目标**(destination)操作数在左，紧邻**操作码**（表明处理器要执行的操作）`#!asm MOV`
...
- 最左边的字段叫做**标签**(label)，用于存储它所表示的内存位置的**符号名称**(symbolic name)
    - 所有的标签必须以字母或以下特殊符号开头：`@`、`$`、`-`、`?`
    - 标签长度不超过 35 个字符
- 右边的下一个字段是**操作码**(opcode)字段，用于存储指令
- opcode 右边是**操作数**(operand)字段，包含为 opcode 使用的信息
...
下图展示了使用 `#!asm MOV` 指令时所有可能的数据寻址模式的变体（几乎能够在所有 Intel 微处理器版本中找到）：

<div style="text-align: center">
    <img src="images/C3/3.png" width=60% />
</div>


### Register Addressing

- **寄存器寻址**(register addressing)是数据寻址中最常用的形式
    - 一旦知道寄存器的名称，就很容易使用这种模式了
- 微处理器包含以下这些用于寄存器寻址的
...
- 对于同一条指令，必须使用相同大小的寄存器
    - 比如不能在同一指令下混用 16 位和 32 位的寄存器，比如 `#!asm MOV EAX, BX`（<span style="color:red">Error: operand type mismatch</span>）
...
- “**立即数**(immediate)”意味着数据在内存中紧跟在十六进制操作码之后
    - 立即数据是常量数据
    - 传输自寄存器或内存的数据是变量数据
- **立即数寻址**(immediate addressing)操作于一个字节或字的数据上

下图展示了 `#!asm MOV EAX, 13456H` 指令操作：

<div style="text-align: center">
    <img src="images/C3/5.png" width=60% />
</div>

- 在符号汇编语言中，某些汇编器使用符号 `#` 来表示立即数，比如 `#!asm MOV AX, #3456H`
- 大多数汇编器不用 `#`，而像这样表示：`#!asm MOV AX, 3456H`

符号汇编器以多种方式表示立即数：

- `H` 表示**十六进制数据**(hexadecimal data)
...
- **十进制数据**(decimal data)的表示无需任何特殊编码或调整
- **二进制数据**(binary data)需要在二进制数后加字母 `B`（一些汇编器要求加的是字母 `Y`）
- 一个或多个 ASCII 编码的字符可以直接表示出来
    - 注意在 ASCII 数据中使用**撇号**(aposttrophe)（\`）而非单引号(single quotation mark)（`'`）
...
- 除了使用寄存器或指令中的操作数外，还可以寻址**内存**中任意位置的操作数
- 当操作数位于内存中时，寻址方式规定了如何通过结合寄存器中的四个参数和/或指令内的常数来计算该操作数的有效内存地址
- 简而言之，**有效地址**(effective address)是相对于段基址的一个偏移量，即：
...
- 地址是通过**将偏移量(displacement)加到**默认的数据段地址或替代段地址上形成的
- **直接数据寻址**的两种基本形式：
...
使用 `#!asm MOV` 指令进行直接寻址，在数据段内的内存位置与 `AL`（8位）、`AX`（16位）或 `EAX`（32位）寄存器之间传输数据，通常为 3 字节长的指令。

例如，从数据段内存位置 `DATA`（`1234H`）加载 `AL`
...
如图所示，该指令将内存地址 `11234H` 的内容复制到 `AL` 寄存器中。

- 在实模式操作系统中，线性地址由偏移地址 `1234H` 与数据段地址 `10000H`（`1000H` * `10H`）相加构成
...
**偏移寻址**与直接寻址几乎相同，区别在于指令宽度为 4 字节而非 3 字节。
...
---
既然大多数指令都采用偏移寻址，为何还要用到直接寻址呢？理由如下：
...
- 累加器（`AX`/`EAX`）的特殊地位
    ...
    - 因此，为最关键的寄存器以及其最频繁的操作（与内存交换数据）提供一条“快速路径”是合理的

---
采用直接数据寻址模式的编译器会访问在编译时确定的静态地址（比如全局变量）。
...
- 若 `BP` 寄存器用于内存寻址，则默认采用**栈段**(stack segment)
- 这些设定被视为这四个索引和基址寄存器的默认配置
- 对于 80386 及更高版本的处理器：
    - `EBP` 默认在栈段中寻址内存
    - `EAX`、`EBX`、`ECX`、`EDX`、`EDI` 及 `ESI` 默认在数据段中寻址内存
...
一把来说，给定内存地址处的数据的预期大小可以从指令中推断出来。例如：

- `#!asm MOV [DI], AL` 是一个字节大小的移动指令
- 但 `#!asm MOV [DI], 10H` 则存在歧义

在某些情况下，间接寻址需要通过**大小指示符**(size directive)或**指针指示符**来明确数据的大小：`BYTE PTR`、`WORD PTR`、`DWORD PTR` 或 `QWORD PTR`。这些指示符指明了由内存指针(memory pointer, PTR)所指向的内存数据的大小。

- `#!asm MOV BYTE PTR [DI], 10H` 指令将 `DI` 所指向的位置指定为一个**字节**大小的内存位置
- `#!asm MOV DWORD PTR [DI],10H` 将该内存位置标识为**双字**(doubleword)大小
- 大小指示符仅用于那些通过指针或索引寄存器配合立即数寻址内存位置的指令
- 对于 SIMD 指令，八进制 `OWORD PTR` 表示一个 128 位宽的数字

下面来看大小指示符是如何影响指令的编码的？在 64 位模式下：

<div style="text-align: center">
    <img src="images/C3/13.png" width=50% />
</div>

- 间接寻址常使程序能够引用内存中的表格数据。
- 下面展示了这个歌表格，以及用于按顺序寻址表中每个位置的 `BX` 寄存器

    <div style="text-align: center">
        <img src="images/C3/14.png" width=50% />
    </div>

- 要完成此任务，需使用 `#!asm MOV` 立即数指令将表的起始地址加载到 `BX` 寄存器中
- 初始化表的起始地址后，利用寄存器间接寻址方式依次存储 50 个样本


### Base-Plus-Index Addressing

- **基址寄存器**(base register)通常存储内存数组的起始位置
- **变址寄存器**(index register)则保存数组中元素的相对位置；每当 `BP` 寻址内存数据时，栈段寄存器与 `BP` 共同生成有效地址
- 对于基址加变址寻址方式
    - 在 8086 至 80286 中，此类寻址采用一个基址寄存器（`BP` 或 `BX`）与一个变址寄存器（`DI` 或 `SI`）间接访问内存，例如：`#!asm MOV DX, [BX + DI]`
    - 80386 及更高版本允许任意两个 32 位寄存器的组合（但 `ESP` 不能作为变址寄存器使用），例如：`#!asm MOV DL, [EAX + EBX]`
...
---
基址加变址寻址的另一个主要用途是访问内存**数组**中的元素。为访问数组中的某个元素，需将数组的起始地址加载到 `BX` 寄存器（**基址**），并将待访问元素的编号加载到 `DI` 寄存器（**变址**）。下图展示了这一过程：

<div style="text-align: center">
    <img src="images/C3/17.png" width=60% />
</div>

采用基址加变址寻址模式的编译器实际上访问的是**一维数组**。
...
- 在 8086 至 80286 中，内存数据通过将偏移量与基址寄存器（`BP` 或 `BX`）**或**变址寄存器（`DI` 或 `SI`）的内容相加来寻址，例如：`#!asm MOV AX, [DI + 100H]`
- 在 80386 及更高版本中，偏移量可以是 32 位数，并且可以使用任何 32 位寄存器（但 `ESP` 不能用作变址寄存器），例如：`#!asm MOV DL, [EAX + 10H]`
- 实模式下的段长度为 64 KB
...
采用寄存器相对寻址模式的编译器会访问一个**结构体**(structure)：
...
- 仅适用于 80386 至 Core2 微处理器
    - 使用两个 32 位寄存器（基址寄存器和变址寄存器）来访问内存
- 第二个寄存器（变址）会乘以一个**比例因子**(scale factor)
    - 比例因子可选值：1x、2x、4x、8x
- 默认比例因子为 1x，故无需特别注明
- 偏移量是可选的
...
- 64 位模式支持相对于 64 位指令指针（**RIP**）的数据寻址，以便在平面内存模型中定位线性地址
- **RIP 相对寻址**(RIP relative addressing)的语法和寄存器相对寻址几乎一样：`[Base + Displacement]`，不同之处在于基址寄存器是 RIP 而非通用寄存器
- RIP 相对寻址采用带符号的 32 位偏移量；下一条指令的有效地址即为该 32 位值进行**符号扩展**后与 RIP 中的 64 位值相加之和，例如：

    <div style="text-align: center">
        <img src="images/C3/26.png" width=50% />
    </div>

- 使用 RIP 相对寻址使得**位置无关代码**(position-independent code)更小且更简单，其中所有代码和数据都需要在 32 位偏移内可寻址


### Canonical Addressing and Canonical Form

- 在 x86-64 系统中，线性地址（虚拟地址）逻辑上长度为 64 位
- AMD 在设计 x86-64 架构时认为，完整的 64 位地址空间过大且实现成本高昂，因此他们定义了 48 位（采用 4 级分页时）或 57位（采用 5 级分页时）的有效地址空间，这些地址必须遵循特定的规范地址规则，例如：
...
- 在 64 位模式下，一个地址若其第 63 位至最高有效位全部设置为 1 或全部为 0，则被视为**规范形式**(canonical form)
- 对于 48 位的线性地址，规范地址必须将其第 48 位至 63 位设置为 0 或 1（取决于第 47 位是 0 还是 1）
...
- 如果内存地址为非规范形式，则会产生**一般保护异常**(general-protection exception)（#GP），例如：`#!asm MOV RAX, [1122334455667788H]`
- 通过检查规范地址格式，系统能够防止软件利用指针未使用的高位进行其他用途
- 在特定处理器实现上遵循规范地址格式的软件，可以在支持更大虚拟地址空间的长模式实现中无需修改直接运行
...
???+ abstract "总结"
    ...
    === "指令操作数"
        ...
        <div style="text-align: center">
            <img src="images/C3/27.png" width=50% />
        </div>

    === "内存寻址模式"

        <div style="text-align: center">
            <img src="images/C3/28.png" width=50% />
        </div>
...
??? question "思考"
    ...
    === "题1"
        ...
        === "解答"
            ...
            B。汇编器足够聪明，能把 `ESP` 作为基址，`EBX` 作为变址。
...
- 我们可以使用 `#!asm FAR PTR` 指示符来实现远程跳转，例如：`#!asm JMP FAR PTR START`
...
- 例如，`#!asm JMP NEAR PTR [BX]` 指的是数据段内偏移地址由 `BX` 寄存器包含的内存位置
    - 在此偏移地址处存放的是一个 16 位数，用作段内跳转的偏移地址
...
- `#!asm PUSH` / `#!asm POP` 是一对重要的指令，用于在 LIFO 的栈内存中存储和检索数据
- `#!asm PUSH` 和 `#!asm POP` 指令有六种形式：
    - **寄存器**寻址：允许任何 16 位寄存器的内容在栈上进行传输
    - **内存**寻址：`#!asm PUSH` 和 `#!asm POP` 指令将 16 位或 32 位内存位置的内容存入栈，或将栈数据弹出到内存位置
    - **立即数**寻址：允许将立即数压入堆栈，但不支持从堆栈中弹出立即数
    - **段寄存器**寻址允许将任何段寄存器的内容压入栈或从栈中移除
        - `CS` 可以被压入，但绝不允许将数据从栈弹出至 `CS`
    - **标志位**可以从栈中压入或弹出
    - **所有寄存器**的内容均可被压入或弹出

- 从 80286 到 Core2 处理器均可使用 `#!asm PUSH` 和 `#!asm POP` 立即数以及 `#!asm PUSHA` 与 `#!asm POPA`（所有寄存器）
...
- 每当一个字的数据被压入栈时：
   ...
   - 低 8 位存放在 `SP-2` 所指向的位置
    ...
    - `SP` / `ESP` 寄存器始终指向位于栈段内的内存区域

- 在保护模式操作下，`SS` 寄存器持有一个选择符，用于访问定义栈段基地址的描述符
- 当数据从堆栈中弹出时：
    - 低 8 位从 `SP` 所指向的位置移除
...
- `#!asm PUSH` 指令将数据传送到堆栈：
    - 8086 至 80286 处理器传送 2 字节，但不会传送 1 字节数据
    - 80386 及更高版本支持传送 2、4 或 8 字节数据

- `#!asm PUSHA`（push all）指令将内部寄存器组的内容（段寄存器除外）复制到栈中
    - 按照以下顺序将寄存器内容压入堆栈：`AX`、`CX`、`DX`、`BX`、`SP`（**原始值**）、`BP`、`SI` 和 `DI`
    - 对于 `SP` 寄存器所压入的值，是在推送第一个寄存器之前其原有的数值
    - 需要 16 字节的栈内存空间来存储所有八个 16 位寄存器；在所有寄存器被压入后，`SP` 寄存器值要减去 16

    <div style="text-align: center">
        <img src="images/C3/36.png" width=30% />
    </div>

    - 当必须保存 80286 及以上型号处理器的整个寄存器组时，`#!asm PUSHA` 非常有用

- `#!asm PUSHAD`（push all double）指令存在于 80386 至 Pentium 4 处理器中，它将所有 32 位寄存器压入栈中（因此需要 32 字节的堆栈存储空间）

- `#!asm PUSHA` 与 `#!asm PUSHAD` 引用相同的操作码（`0x60`）：
   - 当操作数大小为 16 位时使用 `#!asm PUSHA`
   - 当操作数大小为 32 位时使用 `#!asm PUSHAD`
   - 早期 8086/8088 处理器不支持此指令
   - 在 Pentium 4 的 64 位操作模式下无法使用

- `#!asm PUSHF`（push flags）指令将标志寄存器的内容复制到栈中


### POP

- `#!asm POP` 是 `#!asm PUSH` 的逆操作
- `#!asm POP` 从栈中移除数据，并将其放入目标 16 位寄存器、段寄存器或 16 位内存位置上，但不可作为立即数使用
-  `#!asm POPA`（pop all）从栈中移除 16 字节的数据，并按照以下顺序将它们存入指定的寄存器：`DI`、`SI`、`BP`、`SP`、`BX`、`DX`、`CX` 和 `AX`
- `#!asm POPAD`（pop all double）指令在 80386 至 Pentium 4 处理器中用于弹出 32 位寄存器

- `#!asm POPA` 与 `#!asm POPAD` 引用相同的操作码（`0x61`）：
   - 当操作数大小为 16 位时使用 `#!asm POPA`
   - 当操作数大小为 32 位时使用 `#!asm POPAD`
   - 早期 8086/8088 处理器不支持此指令
   - 在 Pentium 4 的 64 位操作模式下无法使用

- `#!asm POPF`（pop flags）从堆栈中移除一个 16 位数，并将其存入标志寄存器