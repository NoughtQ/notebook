...
??? abstract "核心知识"
    ...
    - 中断和异常
        ...
        - 双重错误异常
        - 实模式下处理器处理中断/异常的流程
        - 中断指令：`#!asm INT N`、`#!asm INT3`（断点）、`#!asm INTO`（溢出）
    - 杂项
...
???+ note "比较中断和异常"
    ...
    === "异常"
        ...
        - 每个异常都有一个**助记符**，由一个**井号**（`#`）后跟两个字母和一个可选的错误码（括号内）组成。例如，`#GP(0)` 表示具有错误码 0 的**一般保护异常**
...
- 在**实模式**下，通过将中断类型号乘以 **4** 来确定中断向量的地址（内含 `CS:IP` 值）
    - 例如，`#!asm INT 10H` 指令调用存储在内存位置 `40H`（`10H * 4`）开始的中断服务例程的地址

- 在**保护模式**下，通过将类型号乘以 **8** 来定位**中断描述符**，因为每个描述符长度为 8 字节
...
**栈帧**(stack frame)（也称为激活帧(activation frame)或活动记录(activation record)）是一种内存管理技术，用于支持过程的执行，可以实现子程序的递归功能。
...
- `EBP`（**基指针**(base pointer)）在管理函数调用期间的栈帧中发挥着关键作用：
    ...
    - `EBP` 的值在**整个函数执行过程中保持不变**，而 `ESP`（栈指针）可能会改变
    - 在设置好栈帧后，**函数参数**通过 `EBP` 的**正偏移量**进行访问（比如 `EBP + 8`），而**局部变量**则通过**负偏移量**进行访问（比如 `EBP - 4`），这种一致的访问方式使编译和调试过程更加简单
...
`#!asm ENTER` 和 `#!asm LEAVE` 指令用于**创建**和**释放**调用过程的**栈帧**。
...
- 以下嵌套函数的栈帧：
    ...
    - 用于访问先前函数变量的栈帧指针集合称为**显示**(display)
