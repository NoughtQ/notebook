...
MASM 中的**处理器伪指令**(processor directive/pseudo-instruction)告诉汇编器在汇编过程中启用哪些指令集、处理器类型以及相应的功能。它决定了：
...
- 基于默认操作数/地址大小生成正确的指令编码
    ...
    - 第二条指令中，**操作数大小前缀** `66H` 选择了非默认的操作数大小，而**地址大小前缀** `67H` 改变了内存操作数的默认地址大小
...
- 总的指令长度**不超过 15 个字节**，超出限制的话就会触发**一般保护异常**(general-protection exception)
...
（无前缀的情况下，）第一个字节表示**操作码**(opcode)，负责选择由微处理器执行的运算（比如加法、减法等）。下图展示了多数指令的第一个操作码字节的一般形式：
...
- 剩余 2 位分别表示数据流的**方向**（D）和**数据大小**（W）（字节或字）
    ...
    <div style="text-align: center">
        <img src="images/C4/8.png" width=60% />
    </div>
...
- **MOD**：长度为 2 位，用来指定所选指令的[**寻址模式**](3.md)

    <div style="text-align: center">
        <img src="images/C4/11.png" width=60% />
    </div>

    - 如果 MOD 字段包含 11，则选择的是**寄存器寻址模式**，此时 R/M 字段指定一个寄存器
    - 如果 MOD 字段包含 00、01 或 10，则 R/M 字段选择上图所示的三种**数据内存寻址模式**之一（都是和偏移量相关的）

- **REG/Opcode**：长度为 3 位，指明了**寄存器编号**或有关**操作码**信息
...
- **R/M**：长度为 3 位，指明一个作为操作数的**寄存器**，或者和 MOD 字段结合来编码一种**寻址模式**
    ...
    - 当 R/M = 100 时，指令会新增一个叫做**比例变址字节**(scaled-index byte)的额外字节，用于[比例变址寻址](3.md#scaled-index-addressing)

        <div style="text-align: center">
            <img src="images/C4/15.png" width=60% />
        </div>

        - 80386 之后的处理器有超过 32,000 中 `#!asm MOV` 指令的变体
...
- 可用于**扩展**指令第二字节中的 **REG 和 R/M 字段**，以便引用寄存器 `R8`-`R15`（2^3+1^=16）
...
<div style="text-align: center">
    <img src="images/C4/21.png" width=70% />
</div>
...
**指令前缀**分为四组，每条指令只能使用每个组中的一个前缀：

- 组 1（**锁**/[**重复**](#repeat-prefix)前缀）
    - 0xF0: `#!asm LOCK`
    - 0xF2: `#!asm REPNE` / `#!asm REPNZ`
    - 0xF3: `#!asm REP` / `#!asm REPE` / `#!asm REPZ`
- 组 2（**段**重写前缀）
    - 0x26: `ES` 段重写
    - 0x2E: `CS` 段重写
    - 0x36: `SS` 段重写
...
**LOCK** 前缀会让某些类型的内存读/改/写指令执行**原子操作**，旨在使处理器在多处理器系统中**独占**(exclusive)使用共享内存。该前缀只能与以下写入内存操作数的指令一起使用：

```asm
BTC, BTR, BTS                                            ; 位扫描指令（下一章介绍）
...
- 程序员可以使用**段重写前缀**(segment override prefix)来重写默认段，该前缀要放置在指令开头的字节上
    - 唯一**不能**加前缀的指令是将**代码段寄存器**用于（存储）生成地址的**跳转**和**调用**指令
...
| 汇编语言 | 访问的段 | 默认段 |
| :--- | :--- | :--- |
| `#!asm MOV AX, DS: [BP]` | 数据段 | 栈段 |
| `#!asm MOV AX, ES: [BP]` | 附加段 | 栈段 |
| `#!asm MOV AX, SS: [DI]` | 栈段 | 数据段 |
| `#!asm MOV AX, CS: LIST` | 代码段 | 数据段 |
| `#!asm MOV ES: [SI], AX` | 附加段 | 数据段 |
| `#!asm LODS ES: DATA1` | 附加段 | 数据段 |
| `#!asm MOV EAX, FS: DATA2` | FS | 数据段 |
| `#!asm MOV GS: [ECX], BL` | GS | 数据段 |
...
使用**操作数大小重写前缀**(operand-size override prefix)来混合 16 位、32 位和 64 位数据：

- **REX**（**REX.W**）前缀可以指定 **64 位**操作数大小
- **66H 前缀**指定 **16 位**操作数大小
- REX 前缀的优先级高于 66H 前缀
- **默认操作数大小**由当前**操作模式**定义，但操作数大小重写前缀可以改变默认操作数大小
...
???+ example "例子"
    ...
    因为默认操作数大小为 32 位，所以第二条指令里即便用了 `#!asm DWORD PTR`，也没有出现额外的指令前缀字节。


#### Address-Size Override Prefix

内存操作数的**默认地址大小**同样由当前**操作模式**确定，但可以通过**地址大小重写前缀（67H）**来重写。
...
??? info "VEX Prefix (Vector Extensions)"
...
??? info "Escape Sequence / Opcode"

    由于架构定义的指令超过 256 条，因此必须定义多个不同的操作码映射。具体做法是：通过使用一个**转义操作码字节（0FH）**或**两字节的转义（0F38H，0F3AH）**来扩展编码空间，以提供其他操作码。
...
- `#!asm LEA`：加载**近指针**（偏移量）
- `#!asm LDS`、`#!asm LES`、`#!asm LFS`、`#!asm LGS`、`#!asm LSS` 加载**远指针**（段选择器和偏移量）
...
`#!asm LEA` 的作用是将操作数指定的数据的偏移地址加载到 16 位或 32 位寄存器中。

???+ note "比较 `LEA` 与 `MOV`"

    - `#!asm LEA BX, [DI]` 将 `[DI]`指定的**偏移地址**（此处就是 `DI` 值本身）加载到 `BX` 寄存器中
    - `#!asm MOV BX, [DI]` 将 `[DI]` 地址指向的内存位置存储的**数据**加载到寄存器 `BX` 中

`#!asm SEG` 和 `#!asm OFFSET` 指令返回内存位置的**段**和**偏移值**。
...
- `#!asm OFFSET` 指令比 `#!asm LEA` 指令**更高效**
    ...
    - 原因：实际上第一条指令被汇编为**立即数移动指令**，效率更高（比如 `#!asm MOV BX, 0x9`）
...
- 先从内存中检索到的**偏移地址**加载到指定的 16 或 32 位寄存器中
- 再从内存中检索到的**段地址**或**段选择器**加载到 `DS` 或 `ES` 中
- **32 位远指针**：16 位段 + 16 位偏移
- **48 位远指针**：16 位选择器 + 32 位偏移

???+ example "例子"

    当 `DS=1000H` 且 `DI=1000H` 时，`#!asm LDS BX,[DI]` 指令从地址 `11000H` 和 `11001H` 加载寄存器 `BX`，从地址 `11002H` 和 `11003H` 加载寄存器 `DS`。下面展示了在 `DS` 变为 `3000H` 和 `BX` 变为 `127AH` 之前的情况：

    <div style="text-align: center">
        <img src="images/C4/30.png" width=70% />
    </div>

在 80386 及以上版本中，增加了 `#!asm LFS`、`#!asm LGS` 和 `#!asm LSS` 指令。规则同 `#!asm LDS` 和 `#!asm LES`，但分别加载 `DS`、`ES`、`FS`、`GS` 或 `SS` 段寄存器。

???+ example "例子"

    远地址可以通过汇编器存储在内存中，其中最有用的加载指令是 `LSS` 指令。下面通过使用 `#!asm LSS` 指令同时加载 `SS` 和 `SP` 来重新激活旧栈区：

    <div style="text-align: center">
        <img src="images/C4/32.png" width=60% />
    </div>

    >这里用到了 `#!asm CLI`（禁用中断）和 `#!asm STI`（启用中断）指令。


## String Data Transfers

- **字符串数据传输指令**(string data transfer instructions)：`#!asm LODS`, `#!asm STOS`, `#!asm MOVS`, `#!asm INS`, `#!asm OUTS`
- [**字符串比较指令**](5.md#string-comparisons)(string comparison instructions)：`#!asm SCAS`, `#!asm CMPS`

>注：这些指令的前 2-3 个字母表明了指令的功能，并且 "S" 代表字符串。

- 每个指令都允许以**单字节、字或双字**的形式进行数据传输或比较，并隐式使用 `DI` / `EDI`, `SI` / `ESI` 或同时使用两个寄存器来寻址内存
...
- `#!asm REP` 和 `CX` / `ECX`：重复前缀（`#!asm REP`）使得指令可以重复 n 次，其中 n 是存储在 `CX` / `ECX` 中的值
...
- 在执行字符串指令期间，内存访问通过 `DI` 和 `SI` 寄存器进行
    - `DI` 偏移地址访问**附加段**（`ES`）中的数据（目标字符串）
    - `SI` 偏移地址访问**数据段**（`DS`）中的数据（源字符串）

- 在 32 位模式下操作时，使用 `EDI` 和 `ESI` 寄存器代替 `DI` 和 `SI`，此时字符串能使用整个 4G 字节保护模式地址空间中的任何内存位置


### Direction Flag

**方向标志**(direction flag)（D，位于**标志寄存器**中）在字符串操作期间选择 `DI` 和 `SI` 寄存器的**自动增减**操作。该标志仅与字符串指令一起使用。可以使用以下指令改变该标志：

- `#!asm CLD` 指令选择自动递增模式，清除 D 标志
- `#!asm STD` 指令选择自动递减模式，设置 D 标志


### Repeat Prefix

字符串原语指令仅处理单个内存值或一对值。若添加了一个**重复前缀**(repeat prefix)，则指令会使用 `CX` 或 `ECX` 作为**计数器**，重复执行指令，从而达到使用单个指令来处理整个数组的目标。下面列举了可用的重复前缀：

- `#!asm REP`：当 `ECX > 0` 时重复
- `#!asm REPZ`，`#!asm REPE`：当 `ZF = 1` 且 `ECX > 0` 时重复
- `#!asm REPNZ`，`#!asm REPNE`：当 `ZF = 0` 且 `ECX > 0` 时重复

??? example "示例：拷贝一个字符串"

    在以下示例中，`#!asm MOVSB` 将 10 个字节从 `string1` 移动到 `string2`。当 `#!asm MOVSB` 重复执行时，`ESI` 和 `EDI` 会自动增加，此行为受方向标志控制。
...
`#!asm LODS` 从源位置 `DS:SI` 传输一个字节、字或双字到 `AL`、`AX` 或 `EAX`（**隐式操作数**(implicit operands)，即在操作数或操作码中未明确提及的操作数）。可用后缀指定操作数的大小。
...
??? example "例子"
    ...
    下面展示了当 `DS=1000H`，`SI=1000H`，`D=0` 时 `#!asm LODSW` 指令的操作，对应在 `AX` 从内存加载后，但在 `SI` 增加 2 之前的时间节点：
...
当与 `#!asm REP` 前缀一起使用时，`#!asm STOS` 指令可用于将单个值填充到字符串或数组的所有元素中。
...
该指令是唯一一个可以实现**内存到内存传输**(memory-to-memory transfer)的合法指令。
...
- 使用 `DX` 或 `EDX` 作为源操作数来指定 **I/O 地址**或 **I/O 端口**
- 目标操作数是由 `ES:DI` 或 `ES:EDI` 寻址的内存位置
- 指令的两种形式：
    - **显式操作数形式**(explicit-operands form)：允许显式指定源操作数和目的操作数
        - 例如，`#!asm INS WORD PTR [DI], DX`

    - **无操作数形式**(no-operands form)提供了 `#!asm INS` 指令的字、字和双字版本的“简短形式”
        - `#!asm INSB`：输入 8 位 I/O 数据，并将其存储在由 `DI` 索引的内存位置
...
- **显式操作数形式**：允许显式指定源操作数和目的操作数，例如 `#!asm OUTS DX, WORD PTR [SI]`
    - 源操作数应为 `DS:SI` 或 `DS:ESI`，而目的操作数必须是 `DX`

- **无操作数形式**：提供了 `#!asm OUTS` 指令的字节、字和双字版本的“简短形式”
...
`#!asm XCHG` 指令是一种具有双输出的罕见指令。它的作用是**将寄存器的内容与任何其他寄存器或内存位置交换**，但不能和段寄存器交换，或者交换内存到内存的数据。

- 交换的数据可以是字节、字、双字或四字，并且可以使用除立即寻址以外的任何寻址模式
- 当使用内存寻址模式和汇编器时，哪个操作数寻址内存无关紧要，例如 `#!asm XCHG AL, [DI]` 与 `#!asm XCHG [DI], AL` 相同
- 其中使用 16 位 `AX` 寄存器与另一个 16 位寄存器进行 `#!asm XCHG` 操作，是最有效的交换；此指令占用 **1 个字节**的内存
- 该指令**不会改变任何标志位**
...
另外，`#!asm XCHG` 对于实现进程同步中的**信号量**(semaphore)很有用。
...
- `#!asm LAHF`指令将 `EFLAGS` 寄存器的低 8 位传输到 `AH` 寄存器
    - `#!asm AH := EFLAGS(SF:ZF:0:AF:0:PF:1:CF)`
        - 包括：符号标志（`SF`）、零标志（`ZF`）、辅助进位标志（`AF`）、奇偶标志（`PF`）和进位标志（`CF`）
...
`#!asm XLAT`（**表查找转换**(table look-up translation)）指令

- 使用隐式操作数 `AL`，`BX`
    - `AL` 作为输入时表示的是相对表（`[BX]`）的**偏移量**，而作为输出时将该位置的表内容（`[BX + AL]`）复制到 `AL` 寄存器中
    - `seg:[BX]` 是表的基址；默认段为 `DS`，它可能被段前缀重写

- 该指令等价于 `#!asm MOV AL, [seg:BX + AL]`，但需要注意 `[seg:BX + AL]` **不是一个合法的内存操作数**，只有 `#!asm XLAT` 会接受它
- 该指令常用于将一种格式的数据翻译成另一种格式，下面以“把菜单中食物的索引转换为食物的价格”为例：
    - 首先，为包含价格的表预留 256 字节
...
??? note "输入和输出端口"
    ...
    如屏幕、显示器、键盘、鼠标、硬盘和网络等外部设备通过**输入端口**(input ports)和**输出端口**(output ports)连接到数据总线。每个输入或输出端口都有一个**唯一的地址**，就像内存中的每个字节单元都有一个唯一的地址一样。
    ...
    - 输出端口
        ...
        - 如果地址等于端口地址，且控制总线上有**写信号**，**锁存器**(latch)将存储数据总线上的值
    ...
    - 输入端口
        ...
        - 当地址总线等于输入端口的固定地址，且控制总线上有**读信号**时，三态缓冲器被启用
...
<!-- - 指令通常存储在 **ROM** 中，其端口号由于只读存储器的特性而**永久固定**
...
- I/O 设备（端口）的寻址方式分为以下两类：
    ...
    - **可变端口寻址**(variable-port addressing)：在 `AL`、`AX` 或 `EAX` 与 **16 位**端口地址之间进行数据传输，例如 `#!asm IN AL, DX`，`#!asm OUT DX, AX`
        - I/O 端口号存储在**寄存器 `DX`**（`0000h` 至 `FFFFh`）中，可以在程序执行过程中改变
...
??? example "例子"
    ...
    下面是一个点击电脑中扬声器的程序。扬声器（仅在 DOS 中）通过访问 I/O 端口 `61H` 进行控制。如果先设置该端口最低两位（11），然后清除（00），则会在扬声器上听到点击声。
...
`#!asm MOVZX`（移动和**零扩展**(move and zero-extend)）和 `#!asm MOVSX`（移动和**符号扩展**(move and sign-extend)）指令存在于 80386 及之后的处理器的指令集中。下图展示了这两条指令的操作细节：
...
`#!asm BSWAP`（**字节交换**(byte swap)）指令**反转** 32 位或 64 位寄存器操作数的**字节顺序**，可用于**在大小端形式之间转换数据**。
...
- 64 位：位 7:0 与位 63:56 交换，位 15:8 与位 55:48 交换，位 23:16 与位 47:40 交换，位 31:24 与位 39:32 交换
    - 注意：64 位模式下的指令的默认操作数大小为 32 位，需要使用 `#!asm REX` 前缀来访问额外的 64 位寄存器（`R8`-`R15`）
...
!!! warning "注意"
    ...
    对 **16 位寄存器**应用 `#!asm BSWAP` 指令的结果是**未定义的**。所以要想交换 16 位寄存器的字节，请使用 `#!asm XCHG` 指令。例如要交换 `AX` 寄存器的字节，请使用 `#!asm XCHG AL, AH` 指令。


### CMOV

`#!asm CMOVcc` 类型的指令在 `EFLAGS` 寄存器（`CF`、`OF`、`PF`、`SF` 和 `ZF`）的状态标志处于指定状态（或条件）时执行移动操作。每条指令都与一个**条件码**(condition code, cc)相关联，以指示正在测试的条件。
    
- 只有在条件为真时才移动数据
- 如果条件不满足，则不执行移动，并且继续执行 `#!asm CMOVcc` 指令之后的指令
- 目标操作数仅限于 16 位、32 位或 64 位寄存器，但源操作数可以是 16 位、32 位或 64 位寄存器或内存位置
...
- 而对于条件移动（例如 `#!asm CMOVE eax, edx`），CPU 不需要猜测哪段代码将被执行，从而**避免了预测错误的分支成本**
...
- `#!asm DB`：定义**字节**(define byte)
- `#!asm DW`：定义**字**(define word)
- `#!asm DD`：定义**双字**(define doubleword)

如果一个数值协处理器(numeric coprocessor)在系统中执行软件，那么还有 `#!asm DQ`（定义四字）和 `#!asm DT`（定义十字节）伪指令可用。这些伪指令使用符号名称标记内存位置并指示其大小。

另外，`#!asm DUP` 伪指令允许对相同值进行多次初始化，例如：`#!asm DATA1 DB 0, 0, 0, 0, 0` 等价于 `#!asm DATA2 DB 5 DUP(0)  ; reserves 5 bytes of 0`。

使用问号（`?`）作为 `#!asm DB`、`#!asm DW` 等伪指令的操作数来**为内存预留空间**。汇编器会预留一个位置，不会将其初始化为任何特定值。

`#!asm ALIGN` 指令将下一个数据元素或指令**对齐**到参数的倍数地址，该参数必须是小于或等于段对齐(segment alignment)的 2 的幂。
...
- `#!asm EQU`
    - 相等伪指令（`#!asm EQU`）将数字、ASCII 或标签等价于另一个标签，用于定义**常量**
...
???+ example "例子"
    ...
    1.  使用 `#!asm ORG` 伪指令设置位置
    2.  使用 `#!asm THIS` 伪指令将一个字地址标签赋给一个字节标签
    3.  使用 `#!asm SEGMENT` 伪指令定义一个程序段
...
`#!asm PROC` 和 `#!asm ENDP` 伪指令表示一个必须赋予名称的**过程**(procedure)（**子程序**(subroutine)）的开始和结束。
...
`#!asm PROC` 伪指令可以跟随 `#!asm NEAR` 或 `#!asm FAR`（后者仅在 32 位系统中有效）
...
- `#!asm MACRO` 和 `#!asm ENDM` 伪指令表示一个**宏**(macro)（一个具名的汇编语言语句块）
...
`#!asm INCLUDE` 伪指令用于在汇编过程中将给定文件名的源代码插入到当前源文件中。
...
- 段伪指令不说明段类型，而 `#!asm ASSUME` 伪指令为汇编器提供此信息
- 形式如下：
...
- 将 `#!asm ASSUME` 伪指令**放在程序中所有过程之前**是理想的位置，因为程序中声明为段的段指针很少改变（除了过程入口和出口处）


#### END

- `#!asm END` 伪指令用于通知汇编器**某个模块的结束**
...
- 内存模型是 MASM 独有的
- `#!asm .MODEL` 伪指令包括 MASM 实模式下六个内存模型，即 `tiny`、`small`、`compact`、`medium`、`large` 和 `huge`
- 对于**保护模式**，还可使用 `flat` 模型
- 使用如 `#!asm @DATA`、`#!asm @STACK`、`#!asm @CODE` 的特殊伪指令来识别各种段
