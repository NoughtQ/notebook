...
- MASM 的伪指令能够指示汇编器如何处理操作数或程序的一部分
    - 一些结果存储在内存中，另一些则不存储
...
- 如果一个数值协处理器(numeric coprocessor)在系统中执行软件，`#!asm DQ`（定义四字）和 `#!asm DT`（定义十字节）伪指令也常见
- 这些伪指令使用符号名称标记内存位置并指示其大小
- `#!asm DUP` 伪指令允许对相同值进行多次初始化，例如：
    - `#!asm DATA1 DB 0, 0, 0, 0, 0` 等价于
    - `#!asm DATA2 DB 5 DUP(0)  ; reserves 5 bytes of 0`

- 使用问号（`?`）作为 `#!asm DB`、`#!asm DW` 或 `#!asm DD` 伪指令的操作数来为内存预留空间；汇编器会预留一个位置，不会将其初始化为任何特定值
...
`#!asm PROC` 和 `#!asm ENDP` 伪指令表示一个必须赋予名称的过程(procedure)（子程序(subroutine)）的开始和结束。
...
- `#!asm PROC` 伪指令必须跟随着一个 `#!asm NEAR` 或 `#!asm FAR`（仅在 32 位系统中有效）
...
- `#!asm MACRO` 和 `#!asm ENDM` 伪指令表示一个宏（一个命名的汇编语言语句块）
...
- 将 `#!asm assume` 伪指令放在程序中所有过程之前是理想的位置，因为程序中声明为段的段指针很少改变（除了过程入口和出口处）


#### END

- `#!asm END` 伪指令用于通知汇编器某个模块的结束
