...
??? code "代码汇总"
	...
	准备实习的时候把常见的一些排序算法用 C++ 写了一遍，供读者参考。可能写的不是很好，若发现任何 bug 欢迎指出！
	...
	void bubble_sort(vector<int>& arr) {
		int n = arr.size();
		for (int i = 0; i < n - 1; ++i) {
			for (int j = 0; j < n - 1 - i; ++j) {  // caution: last i elements are sorted!
				if (arr[j] > arr[j + 1]) {
					swap(arr[j], arr[j + 1]);
				}
			}
	...
	// unstable
	void quick_sort(vector<int>& arr, const int lb, const int rb) {
		if (lb >= rb) return;
		
		int n = rb - lb + 1;
		int idx = lb + rand() % n, pivot = arr[idx];
		swap(arr[idx], arr[lb]);
		int l = lb + 1, r = rb;
		...
		swap(arr[r], arr[lb]);
		quick_sort(arr, lb, r - 1);
		quick_sort(arr, r + 1, rb);
	}

	void percolate_down(vector<int>& elems, const int p, const int n) {
		int i, x = elems[p], child;
		for (i = p; i * 2 + 1 < n; i = child) {
			child = i * 2 + 1;
			if (child < n - 1 && elems[child] < elems[child + 1]) {
				++child;
			}
			if (x < elems[child]) {
				elems[i] = elems[child];
	...
	void merge(vector<int>& arr, const int ls, const int rs, const int re) {
		int p = 0, l = ls, r = rs;
		vector<int> tmp(re - ls + 1);
		while (l < rs && r <= re) {
			if (arr[l] < arr[r]) {
				tmp[p++] = arr[l++];
			} else {
		...
		while (l < rs) tmp[p++] = arr[l++];
		while (r <= re) tmp[p++] = arr[r++];
		for (int i = 0; i < p; ++i) {
			arr[ls + i] = tmp[i];
		}
	}

	void merge_sort(vector<int>& arr, const int l, const int r) {
		if (l >= r) return;
		int mid = l + ((r - l) >> 1);
	...
	int main() {
		srand(time(nullptr));
		int n;
		cout << "Please input number of elements in the array: ";
		cin >> n;
		vector<int> nums(n);
		for (int i = 0; i < n; ++i) {
			nums[i] = rand() % INT_MAX;
...
+ 仅考虑**内部排序(internal sorting)**（即整个排序能在主存中完成）
