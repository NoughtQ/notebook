...
- 链表删除操作的随机化算法
    ...
    - 预期删除次数：$\Theta(\log N)$，下面给出证明：
        ...
        ??? proof "证明"
            ...
            >感谢 [@tanginrattay](https://github.com/tanginrattay) 提供的正确思路。
            ...
            - 对于第 $k$ 个节点，它只有在被选中的时候才会贡献一个操作步骤（如果是因为它前面的节点被选中而被动删除，那么该节点就没有任何贡献）
            - 因此第 $k$ 个节点的贡献一步的概率为 $\dfrac{1}{k}$，总的期望次数就是：

                $$
                \sum\limits_{k=1}^N \dfrac{1}{k} = \ln N + \gamma \approx \Theta(\log N)
                $$
