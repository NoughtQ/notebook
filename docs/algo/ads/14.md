...
- 指令级并行：在一个CPU内，多条指令可以同时执行，包括了[计组](../../sys/co/index.md)课程中提到过的流水线CPU、乱序、多发射等（后面两者将会在体系课程中详细介绍）。
...
>【26.01.04】先前关于 $T(n)$ 和 $D$ 的表述有误，在此感谢 [@RanderDouble](https://github.com/RanderDouble) 的指正🙏
...
!!! note "结论（类似上面的定理，~~但至少是我看得懂的~~）"
    ...
    其中$W$为工作量，$D$为（分析并行算法时用的满二叉树）深度，也就是处理器数量为无穷多时，在最坏情况下的执行时间。
...
???+ info "根据上一种算法想到的划分方式"
    ...
    - 对于每个子问题，**并行递归地寻找最大值**，因此所需时间为$T(\sqrt{n})$，工作量为$W(\sqrt{n})$
