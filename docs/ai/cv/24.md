...
**动态范围**(dynamic range)：某一定量（比如亮度）最大值和最小值之间的比例。

???+ example "例子"

    - 10:1        照片打印（在光泽纸张上比例更高）
    - 256:1       8 位 RGB 图像
    - 1000:1      LCD 显示器
...
- **失焦**(defocus)：主题不在景深范围内

    <div style="text-align: center">
        <img src="images/lec24/18.png" width=50% />
    </div>

- **运动模糊**(motion blur)：移动的物体或不稳定的相机
...
**彩色化**(colorization)指借助计算机为单色图片或视频添加颜色的过程。

有两种主要的为灰度图像上色的方法：

- **基于样本**的彩色化(sample-based colorization)：使用样本图像
