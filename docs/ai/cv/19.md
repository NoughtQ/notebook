...
??? note "监督学习 vs 无监督学习"
    ...
    - **监督学习**(supervised learning)：
        ...
        - 目标：习得一个 x -> y 的函数
        - 例子：分类、回归、目标检测、语义分割、图像描述等
    - **无监督学习**(unsupervised learning)：
        - 数据：x（只有数据，没有标签）
        - 目标：习得数据背后的隐藏结构
        - 例子：聚类、降维(dimensionality reduction)、特征学习、密度估计等
...
- **预文本任务**(pretext tasks)：
    ...
    - 我们能自动生成用于预训练任务的标签
    - 预文本任务的类别：
        - **生成式**(generative)：预测部分输入信号
            - 例子：自编码器（稀疏、去噪(denoising)、掩码）、自回归(autoregressive)、GAN、上色(colorization)、修复(inpainting)
        - **判别式**(discriminative)：预测关于输入信号的某些内容
            - 例子：上下文预测、旋转、聚类、对比
        - **多模态**(multimodal)：使用除 RGB 图像外的额外信号
            - 例子：视频、3D、音频、语言
