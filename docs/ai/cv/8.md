...
??? abstract "核心知识"
    ...
    - 深度估计：计算深度图
        - 立体匹配
            - 简单情况：极线水平时计算视差（和深度成反比）
            - 非水平情况可通过矫正（单应变换）变成水平的
            - 不考：通过能量最小化寻找最佳立体匹配
...
- **被动深度感知**(passive depth sensing)：通过 RGB 图像计算深度（[立体视觉](#stereo-vision)）
...
若极线不是水平的，那么需要进行**立体图像矫正**(stereo image rectification)，即将图像平面重新投影到一个与相机中心之间的线平行的公共平面上。这一操作需要**两个[单应变换](6.md#projective-transformation)**（3x3），每个变换对应一个输入图像重投影操作。
...
基础的立体匹配算法就是找到能够**最小化差异**(dissimilarity)的最佳匹配。
...
- 计算参考图像中每个点深度值的误差，寻找能给出最小误差的深度值
...
- 通过下图看出，点的**法线**与指示函数的**梯度**之间存在密切联系

    <div style="text-align: center">
        <img src="images/lec8/45.png" width=50% />
    </div>
...
- 考虑到对称的话，实际上只需考虑以下 **15 种配置**（右图黄点表示在表面内的点）
