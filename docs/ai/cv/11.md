...
??? info "meme"
    ...
    <div style="text-align: center">
        <img src="images/lec11/69.png" width=20% />
    </div>

---
「神经网络」是一个很宽泛的概念。更准确的叫法是**全连接网络**(fully-connected network)（有时又称为**多层感知器**(multi-layer perceptrons, **MLP**)）。不过在本讲笔记中我们就不做区分了（~~因为还没有引入更多的神经网络类型~~）。
...
实际上这个简单的 $\max$ 函数叫做 ReLU。它是一种**激活函数**(activation function)，而且它经常作为许多问题的默认激活函数（~~显然是因为表现不错~~）。下面罗列了各种激活函数的函数图像：
...
![](images/lec11/10.png){ align=right width=30% }
