...
- 和牛顿法相比
    ...
    - 可以看到，高斯-牛顿法**使用 $J_R^T J_R$ 近似表示 $H_F$**
...
**Levenberg-Marquardt 方法**（以下简称 **LM**）在高斯-牛顿法的基础上引入**正则化**(regularization)，因此
$$
...
为减小离群点对估计带来的影响，我们需要用别的损失函数来替代 MSE，比如 L1，Huber 等。像这些函数被称为**鲁棒函数**(robust function)。

其中，最强大的处理离群点的方法是 **RANSAC**（**随机抽样一致**(random sample concensus)），其关键思想为：
...
过拟合会导致**不适定问题**(ill-posed problem)，即解的数量不唯一（0 个或大于 1 个）或者解不稳定（数据的微小变化会引起解的剧烈变化）的现象。
...
!!! warning "注意"
    ...
    **L1** 比 L2 对离群点表现得**更加鲁棒**。
    
    - 因为即便只有少数离群点，如果离群点距离过远，L2 会放大这一距离，使得模型为了降低整体损失而被迫去拟合这些极端的离群点，从而导致模型偏离大部分正常数据的趋势
    - 而采用 L1 正则化的模型在优化过程中受单个极端值的影响较小，能够更好地保持对数据主体分布的拟合