...
以 ImageNet 数据集诞生为分水岭，已经涌现了诸多现代意义上的计算机视觉算法。
...
现在我们讲到的图像分类器都是基于**机器学习**(machine learning)的方式实现的。这是一种**数据驱动的方法**(data-driven approach)（向机器提供每个类别的许多示例，然后开发学习算法让机器观察这些示例并学习每个类别的视觉外观）。搭建一个图像分类器的步骤如下：
...
- 将已知数据划分为**训练集**和**验证集**，选择在验证集上表现最好的超参数，用于在测试集上评估
...
- **代数**视角：就是一个矩阵和向量间的乘法与加法运算

    <div style="text-align: center">
        <img src="images/lec9/32.png" width=60% />
    </div>

    - 我们可以将偏差向量看成是权重矩阵的最后一列，然后在输入向量后再增加一维，其值设为 1，从而简化计算（类似齐次坐标）
...
- **视觉**视角：
    ...
    - 这些小的分类器可看作是一张张图像（或者说**模板**(template)）（图片底部的一排很模糊的图像），线性分类器要做的就是将图像匹配到与其最接近的模板（通过**内积**计算）
...
- **几何**视角：
    ...
    - 既然是“线性”，自然有许多**非线性**的情况困扰着线性分类器，比如下面这些情况很难用线性分类器做分类：
...
构建一个线性分类器的关键在于选择一个好的 $W$。一般我们会用这样的方法来寻找：
