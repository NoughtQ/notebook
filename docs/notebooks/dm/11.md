...
+ **满m叉树(full m-ary tree)**：每个内部节点有且有m个孩子的有根树([[Chap 5 Induction and Recursion#^69caaa|Chap 5]]介绍过满二叉树)
+ **二叉树(binary tree)**：m = 2
...
>[!warning]
>注意：这里的高度 = fds里的深度，且根的高度 = 0，千万不要搞错！！！

如果有根m叉树的高度为h，且所有叶子的高度为h或h - 1，则这棵树是**平衡的(balanced)**
...
2. 用决策树证明基于比较的排序的时间复杂度([[Chap 7 Sorting#General Lower Bound for Sorting|fds]]已讲过)
