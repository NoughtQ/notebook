...
$$\dfrac{\pi (x)}{x} = \dfrac{x/ \ln x}{x} = \dfrac{1}{\ln x}$$
...
+ 如果$a, b$是正整数，那么$\text{gcd}(2^a - 1, 2^b - 1) = 2^{\text{gcd}(a, b)} - 1$ ^816496
...
+利用4.1节[[Chap 4 Number Theory and Cryptography#^a8d7a5|定理4]]，根据已知条件往回代，最后可以得到形如$x = mk + r$的形式，则所有解为$x \equiv r (\text{mod } m)$
