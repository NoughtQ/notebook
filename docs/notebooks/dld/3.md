...
???+ note "相关知识"
	...
	**高到低**($t_{PHL}$)和**低到高**($t_{PLH}$)的输出信号的改变可能有不同的传播延迟。这里两者<u>只与输出有关</u>，与输入不相关。
...
!!! note "注意"
	...
	对于大规模的电路结构，比如上面的64-1多路复用器，以及之后的算术逻辑单元、寄存器等，我们往往只要画出它封装后的模块即可，而不关注它的细节。
...
!!! question "实例分析：格雷码 $\rightarrow$ 二进制码"
...
具体做法：单元中的$C_{i+1}$的作用改为得到一组关于进位的方程，因此所有的$C_i$都可以被$C_0$和其他变量表示，比如(4位加法器)：
...
>注：这里的概念讲过很多遍了，实在不熟悉的话可以参考以下:point_down:
