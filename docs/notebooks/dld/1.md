...
![](./images/C1/Quicker_20240418_172618.png)

用框图表示：
![](./images/C1/Quicker_20240418_172818.png)

---
**模数转换器(Analog-to-Digital Converters, ADC)**：将模拟信号转换为数字信号，为模拟世界的传感器和数字世界的信号处理搭起了桥梁
![](./images/C1/Quicker_20240418_173157.png)
🌰：温度测量和显示
![](./images/C1/1.png)
### Signals
...
🌰1位信号——电压
![](./images/C1/Quicker_20240418_174155.png)
...
$$
![](./images/C1/Quicker_20240418_184009.png)
...
注意*进位(carry)* 问题
![](./images/C1/Quicker_20240502_154329.png)
#### Subtraction

注意*借位(borrow)* 问题
![](./images/C1/Quicker_20240502_154339.png)
#### Multiplication

![](./images/C1/Quicker_20240502_154348.png)

---
不同基底对应的值
![](./images/C1/Quicker_20240418_184855.png)
...
常见的编码方式
![](./images/C1/Quicker_20240418_193235.png)
...
🌰
![](./images/C1/Quicker_20240418_195128.png)

### Excess 3 Code and 8, 4, -2, -1 Code

![](./images/C1/Quicker_20240418_193951.png)
>注意到：
>+ Excess 3 = BCD + 3
...
![](./images/C1/Quicker_20240502_154845.png)


### Error-Detection

>**错误侦测(error detection)** 通常用于**通道编码(channel coding)** 中
>![](./images/C1/Quicker_20240418_201945.png)
...
+ **奇偶校验位(parity)** 是添加在编码单词上的额外的位，使得整个编码包含1的个数是奇数或者偶数。它能够侦测所有单个位的错误和一些多位错误 ^79ab41
	+ **偶校验(even parity)**：整个编码包含1的个数是偶数
	+ **奇校验(odd parity)**：整个编码包含1的个数是奇数
	🌰：![](./images/C1/Quicker_20240418_203028.png)
+ **校验和(Checksum)**

## Gray Codes

![](./images/C1/Quicker_20240418_203219.png)
>注：**格雷码(Gray code)** 的特殊之处在于：**相邻**的两个数的差异只有**1位**。观察表格发现，格雷码的前半部分和后半部分有*反射(翻转)(reflection)* 的关系

🌰光轴编码器(Optical Shaft Encoder)
![](./images/C1/Quicker_20240418_203632.png)
如果光线经过红色虚线(边界)处，左边用二进制码表示的情况就会出现问题（称为*偏斜(skew)*）：下图所示红色部分的数字都有可能是最终的输出结果
![](./images/C1/Quicker_20240418_203834.png)
而格雷码就可以避免这种错误的输出可能

二进制转格雷码：
>注：
>+ 这里用1表示最高位，索引值越大表示位数越低
...
🌰
![](./images/C1/Quicker_20240418_204502.png)

## Appendix

### Types of Digital Systems

+ 无状态(no state present)——**组合逻辑系统(combinational logic system)**，$\text{Output} = \text{Function(Input)}$
	![](./images/C1/Quicker_20240418_205103.png)
+ 有状态(state present)：$\text{State} = \text{Function(State, Input)}$
	+ **同步时序系统(Synchronous Sequential System)**：状态在离散的时间内更新
	+ **异步时序系统(Asynchronous Sequential System)**：状态随时更新
	$\text{Output} = \text{Function(State) or Output = Function(State, Input)}$
	![](./images/C1/Quicker_20240418_205448.png)

### The Digital Computer 

![](./images/C1/Quicker_20240418_205603.png)
...
![](./images/C1/Quicker_20240502_151539.png)
...
![](./images/C1/Quicker_20240502_152326.png)
...
![](./images/C1/Quicker_20240502_155233.png)

>注：具体内容见[维基百科](https://en.wikipedia.org/wiki/Unicode)
