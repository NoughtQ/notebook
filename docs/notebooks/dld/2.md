...
**代数结构(algebraic structure)**由两个元素的集合$B = \{0, 1\}$，以及三种运算符$+, \cdot, \overline{}$构成。它满足下列基本恒等式
...
根据*SOM & POM*这一小节的[第一个例子](#som-and-pom)，将化简过程倒过来便是函数(SOM)转化为SOP的形式的步骤。观察发现，我们大幅简化了布尔函数，而且实现上也变得更加简单：
...
+ 最小项对应的输入从未出现过
+ 最小项对应的输出从未用过
这样的输出值我们并不关心，因此用$\times$代表**Don't-Cares**，既可以看作1，也可以看作0。在SOP化简中，我们可以把$\times$看作"1"，这样可以形成更大面积的主蕴含项，有助于化简，降低逻辑电路成本。
