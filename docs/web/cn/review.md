...
查表转发的结果可以指明 IP 数据报的下一跳路由器的 IP 地址，但无法指明该 IP 地址所对应的 MAC 地址。因此，在数据链路层封装该 IP 数据报成为帧时，帧首部中的目的 MAC 地址字段就无法填写，该问题需要使用网络层中的**地址解析协议**（**ARP**）来解决。
...
- 三个重要特点：
    ...
    - 交换什么信息：交换路由器自己的**路由表**，即本路由器到所在 AS 中各网络的最短 RIP 距离，以及到各网络应经过的下一跳路由器
    - 何时交换信息：
        - **周期性交换**
...
- **慢开始**(slow-start) + **拥塞避免**(congestion avoidance)
    - 慢开始：一开始向网络**注入的报文段少**，而并不是指拥塞窗口 cwnd 的值增长速度慢
...
- **快重传**(fast retransmit) + **快恢复**(fast recovery)
    - 快重传：使发送方尽快（尽早）进行重传，而不是等重传计时器超时再重传
        ...
        - 对于个别丢失的报文段，发送方不会出现超时重传，也就不会误认为出现了拥塞而错误地把拥塞窗口 cwnd 的值减为 1
...
直观上，超时值应大于 RTT，但 RTT 是变化的，所以按以下方法估计 RTT：
...
<div align=center>
    <img src="images/353.png" width=70% />
</div>
