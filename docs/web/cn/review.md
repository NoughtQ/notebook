...
为了更有效地转发 IP 数据报以及提高 IP 数据报交付成功的机会，TCP/IP 体系结构的网络层使用了**网际控制报文协议**(internet control message protocol，**ICMP**) [RFC 792]。
...
前面介绍的物理层、数据链路层和网络层共同解决了将主机通过异构网络互联起来所面临的问题，实现了主机到主机的通信。然而，在计算机网络中，实际进行通信的**真正实体**，是位于通信两端主机中的**进程**。如何为运行在不同主机上的应用进程提供直接的逻辑通信服务，就是**传输层**的主要任务。传输层协议又称为端到端协议。

<div align=center>
    <img src="images/244.png" width=70% />
</div>

传输层向应用层实体屏蔽了下面网络核心的细节（例如网络拓扑、所采用的路由选择协议等），它使应用进程看见的就好像是在两个传输层实体之间有一条端到端的逻辑通信信道。
