...
- 异常还可以分为：
    - **未检查型异常**(unchecked exception)：包括 `RuntimeException`、`Error` 以及它们的子类，在多数情况下反映了程序中**不可恢复的逻辑错误**
...
- 创建任务和线程：
    ...
    - 作为任务的类实现了 `Runnable` 接口，重写了 `run()` 方法
...
- `Thread` 类：
    ...
    - `#!java Thread.yield()` 静态方法：使当前运行中的线程主动放弃对 CPU 资源的使用权，即进入就绪状态（`Runnable`）
    - `#!java Thread.sleep(long mills)` 静态方法：使线程休眠指定的毫秒数，**不会释放锁**
...
---
使用 Java 对象自带的 `wait()`、`notify()` 和 `notifyAll()` 方法实现线程之间的通信：
...
- 调用 `wait()` 会暂停线程，进入 `WAIT` 状态，同时释放该对象的锁；当线程被通知重启后，将会自动重新获得该锁
