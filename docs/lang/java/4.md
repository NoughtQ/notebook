...
- Java 8 里关于函数式接口的包是 `java.util.function`，里面全部是函数式接口，包括：
    - `Function`：接收参数，并返回结果，主要方法 `#!java R apply(T t)`
    - `Consumer`：接收参数，无返回结果, 主要方法为 `#!java void accept(T t)`
    - `Supplier`：不接收参数，但返回结构，主要方法为 `#!java T get()`
    - `Predicate`：接收参数，返回 `#!java boolean` 值，主要方法为 `#!java boolean test(T t)`
    - 没有 `Operator` 接口，只有类似 `BinaryOperator` 这样的接口
