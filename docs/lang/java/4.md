...
- 异常还可以分为：
    - **未检查型（或非受检）异常**(unchecked exception)：包括 `RuntimeException`、`Error` 以及它们的子类，在多数情况下反映了程序中**不可恢复的逻辑错误**
...
- `#!java finally` 子句：无论程序使用何种方式退出 `#!java try-catch` 块，该子句内的代码都会被执行
    ...
    - ⚠️注意：如果在 `#!java catch` 块中抛出异常（或者 `#!java return` 等情况），则 <u>`#!java try-catch` 块之后的代码不会执行</u>，但是 `#!java finally` 中的代码始终会执行
    ...
    - `#!java finally` 块不能抛出异常
        - 因为如果在 `#!java finally` 块中抛出异常，`#!java try-catch` 块的异常就不能抛出，外部捕捉到的异常就是 `#!java finally` 块中的异常信息，而 `#!java try-catch` 块中发生的真正的异常栈信息则丢失了

- `#!java try` 语句块不可以单独存在，但可以只有 `#!java try` + `#!java finally` 语句块
- 抛出自定义异常异常时带上原始异常信息
- 打印异常信息时带上异常栈
...
- **嵌套**：可以将一套完整的 `#!java try-catch-finally` 结构放入另一个 `#!java try` / `#!java catch` / `#!java finally` 块中，但要注意：
    - `#!java try`：如果内层 `#!java catch` 成功捕获了异常，外层 `#!java catch` 就不会再感知到这个错误
    - `#!java catch`：可以缓减 `#!java catch` 块异常处理失败造成的影响
    - `#!java finally`：不建议这么做，因为其内部 `#!java catch` 块会覆盖原来的异常信息

- 如果父类方法没有声明任何异常，子类在重写该方法时不能声明任何**检查型异常**，但子类依然可以声明抛出任何非检查型异常
...
- `File` 类：用于文件名及其目录路径的包装类
    ...
    - 它封装了文件或路径的属性，但<u>不包含用于从/向文件读取/写入数据的方法</u>
...
- 使用 `java.util.Scanner` 读数据（[前面](2.md#input-the-scanner-class)也详细介绍过）


#### Advanced Text I/O

<div style="text-align: center">
    <img src="images/lec4/38.png" width=60%>
</div>
...
- `#!java Serializable` 接口：
    ...
    - 凡是实现 `Serializable` 接口的类都有一个表示**序列化版本标识符**的变量：`#!java private static final long serialVersionUID`，它是根据类名、接口名、成员方法及属性等生成的一个 64 位哈希字段
    - `#!java static` 字段在序列化时会被跳过，因为它不属于任何对象实例

- `#!java trasient` 关键字：
    - 一个包含不可序列化的实例数据字段的 `#!java Serializable` 的对象**不可以**被序列化
    - 要使此类对象能够被序列化，可以使用 `#!java transient` 关键字标记这些数据字段，以告诉 JVM 在将对象写入对象流时**忽略**这些字段
...
???+ abstract "总结：子类/父类中只有一个实现了序列化接口在序列化/反序列化时的行为"

    - 父类实现了序列化，子类没有显式实现
        - 在 Java 中**序列化属性是具有继承性的**，因此子类会自动被视为“可序列化的”
        - 序列化子类对象时，子类中定义的所有字段以及父类中定义的字段都会被写入字节流
        - 反序列化时，系统会直接从字节流中恢复子类和父类的所有状态，不会调用任何构造函数
    - 子类实现了序列化，但父类没有实现
        - 序列化时只有子类中定义的字段会被保存到字节流中，父类中的字段不会被序列化
        - 反序列化时，子类字段从字节流中读取并还原；而对于父类字段，JVM 会寻找父类中的**第一个非序列化父类的无参构造函数**并运行它，因此父类字段的值为默认值
        - 如果父类没有无参构造函数，反序列化时会抛出 `InvalidClassException`
...
`PipedOutputStream` 和 `PipedInputStream` 分别是管道输出流和管道输入流，它们的作用是**让多线程可以通过管道进行线程间的通讯**。在使用管道通信时，必须将 `PipedOutputStream` 和 `PipedInputStream` 配套使用，并且在不同的线程中使用（否则会引起死锁）。
...
**泛型**(generics)是**参数化类型**(parameterized types)的特性。利用这个特性，我们可以定义一个具有泛型类型的类或方法，**编译器**可以使用具体类型来替换这些泛型类型。其关键优点是能够让错误在**编译时**被检测出来。
...
- **泛型方法**（注意返回类型是**带尖括号的**，表示新**声明**的类型参数，而不是使用类提供的类型参数）
...
- **有界泛型类型**(bounded generic type)
    ...
    ???+ example "例子"
        ...
        `#!java <E extends GeometricObject>` 表示 `E` 必须是 `GeometricObject` 或其子类
...
- 参数化类型**没有**实际类型参数的**继承关系**
    ...
    - 举例：`#!java List<Integer> list = new List<Object>();` 会发生**编译错误**，反之依然
    - 泛型的继承关系需要通过通配符实现
...
- 泛型的**类型擦除**(type erasure)机制
    - 编译器使用泛型类型信息来编译代码，但随后会将其擦除，因此**运行时无法获取泛型信息**
    - 这使得泛型代码能够与使用基本类型的遗留代码**向后兼容**
...
- 限制：
    ...
    - 泛型类不能继承 `java.io.Throwable`，也就是说**异常类不能是泛型类**
...
- 它没有引入新的方法或常量，但它规定 `Set` 实例**不包含重复元素**，即集合中不会存在两个元素 `e1` 和 `e2`，使得 `#!java e1.equals(e2)` 为 `#!java true`
    - 如果重复添加元素，程序不会抛出异常
...
- `LinkedHashSet` 类
    - 继承了 `HashSet` 类，同样是根据元素的 `hashCode` 值来决定元素的存储位置，但是它同时使用**双向链表**维护元素的次序，使得元素看起来像是**以插入顺序保存**的
    - 因为用链表维护顺序，所以在**迭代访问**集合中的全部元素时，性能比 `HashSet` **好**，但是**插入**时性能稍微**差**于 `HashSet`
    - 方法签名同 `HashSet`
...
!!! warning "注意"
    ...
    - 列表可以动态增长或缩小，而数组在创建后是固定的；因此，如果不需要插入或删除元素，最有效的数据结构就是**数组**
    - 不能用 `[i]` 访问 `List` 元素（只能用 `.get(i)`），这一语法专门留给 Java 原生数组使用
...
- `ArrayList`
    ...
    - 基于**动态数组**实现，按**插入顺序**来保存和访问数据
    - 用途：需要通过索引支持**随机访问**，而不在**末尾**之外的位置**插入或删除**元素（虽然支持，但效率不高，所以不建议这么做）
...
!!! warning "注意"
    ...
    - `Collection`：定义了常见容器的**接口**
    - `Collections`：提供一系列操作容器的方法（**工具类**）

`Collections` 类包含了用于**操作**容器(collections)和映射、用于创建**同步**(synchronized)容器类和**只读**(read-only)容器类等的各种**静态方法**。
...
- `TreeMap`
    - 实现了 `SortedMap`，适合以排序顺序**遍历**键（根据键的大小）
    ...
    ???+ note "如何遍历 `Map`"
        ...
        - `foreach` + [Lambda 表达式](#lambda-expressions)（形式最简单）
...
`Arrays` 类包含用于排序和搜索数组、比较数组、填充数组元素、将数组转换为列表等的**静态方法**。
...
- `EnumSet`
    ...
    - `EnumSet` 是一个**抽象类**，只能通过它提供的静态方法来返回 `EnumSet` 的实现类的实例
    - 两种不同的实现：
...
???+ abstract "总结：容器的线程安全性"

    >由 Gemini 3 Pro 生成。

    | 线程安全性 | 典型容器（例子） | 特点简述 |
    | :--- | :--- | :--- |
    | 非线程安全 | `ArrayList`, `LinkedList`, `HashMap`, `HashSet`, `TreeSet`, `TreeMap` | 性能优秀，但多线程并发读写会导致数据冲突或异常 |
    | 早期同步安全 | `Vector`, `Stack`, `Hashtable` | 内部方法均加锁，虽然安全但高并发下性能损耗极大 |
    | 现代并发安全 | `ConcurrentHashMap`, `CopyOnWriteArrayList`, `ConcurrentLinkedQueue` | 针对高并发场景设计，兼顾了安全性与执行效率 |
    | 包装同步安全 | `Collections.synchronizedList`, `Collections.synchronizedMap` | 通过工具类对现有非安全容器进行二次包装实现同步 |
...
- `thread.start()` 会**启动一个全新的线程**，线程进入 `RUNNABLE` 状态（但**不一定立即执行**）；能够实现真正的并发执行
...
- Java 线程的生命周期
    ...
    <div style="text-align: center">
        <img src="images/lec4/39.png" width=70%>
    </div>

    - 其中 `RUNNABLE` 状态包含**就绪**(ready)和**运行**(running)中两个子状态，所以进入 `RUNNABLE` 状态的进程并不一定会立即执行（也许 CPU 核心还在处理其他高优先级的线程，或者时间片尚未分配给该线程）
...
- `Thread` 类：
    ...
    - `#!java Thread.yield()` 静态方法：使当前运行中的线程主动放弃对 CPU 资源的使用权，即进入就绪状态（`Runnable`）；**不会释放锁**
    - `#!java Thread.sleep(long mills)` 静态方法：使调用此方法的当前线程休眠指定的毫秒数，但实际休眠时间可能会超过 `mills`；**不会释放锁**
    - `#!java anotherThread.join()`：强制当前线程（调用该方法的线程）等待另一个线程（`anotherThread`）完成；此时当前线程会**释放其持有的 `anotherThread` 的锁，但其他锁不会释放**
    ...
    - `#!java isAlive()`：用于确定线程状态
        - 如果线程处于**就绪**、**阻塞**或**运行**状态，则返回 `#!java true`
    ...
    - 线程优先级
        ...
        - 高优先级的可运行线程**不一定会**抢占低优先级线程，高优先级只能说明线程获取 CPU 的概率更大
...
**内存可见性问题**：一个线程对共享变量的修改，可能不被另一个线程及时看到。因为每个线程都有自己的工作内存，而共享变量存储在主存中。如果线程对共享变量的修改没有及时刷新到主存就可能出现问题。
...
- `#!java synchronized` 关键字：被修饰的方法在同一时间内只能被一个线程访问
    ...
    - 对于**静态**方法，锁是针对**类**
    - **同步块**(synchronized block)：在执行方法中的代码块时，可以使用同步语句来锁住**任何类型的对象**
    ...
    - 如果在同步块中抛了任何**异常**，都会**在异常抛出之前自动释放锁**，防止单线程异常导致系统死锁，从而避免资源被永久占用
...
- 使用**显式锁**
    ...
    - 锁是 `java.util.concurrent.locks.Lock` 接口的一个实例；该接口声明了获取和释放锁的方法
    - 悲观/乐观锁：
        - 悲观锁：
            - 为避免冲突，每次读写数据前直接上锁
            - 虽然能严格保证数据安全，但线程的阻塞和唤醒会带来上下文切换的开销，影响性能
            - 例子：`#!java synchronized`、`ReentrantLock`
        
        - 乐观锁：
            - 读取和处理数据时完全不会加锁；只有在真正提交更新的那一刻才去检测刚才这段时间里数据有无改动
            - 如果检测到数据发生变化，通常会选择重试或者放弃操作
            - 例子：`java.util.concurrent.atomic` 的各种原子类
...
- **异步结果**：将子任务的管理封装为异步调用，异步调用马上返回，但返回的不是最终结果，而是一个 `Future` 类对象，随后可通过它获得最终的结果
...
---
使用 `Object` 类自带的 `wait()`、`notify()` 和 `notifyAll()` 方法实现线程之间的通信：
...
- 调用 `wait()` 会暂停线程，进入 `WAIT` 状态，同时**释放该对象的锁**；当线程被通知重启后，将会自动重新获得该锁
...
???+ note "**闭包**(closure)"
    ...
    闭包是指一个函数能够捕获并持有其外部作用域中变量引用的现象
    
    - 即使外部作用域的代码已经执行完毕，闭包依然可以访问并使用这些被捕获的变量
    - 主要实现方式：
        - **Lambda 表达式**
        - [**匿名内部类**](3.md#nested-classes)
    - 被闭包捕获的局部变量应该被声明为 `#!java final`，或确保在初始化后不再被重新赋值
    - 若尝试在闭包内部修改外部的局部变量，或者在外部修改已被捕获的变量，编译器会报错

    ??? example "例子"

        ```java
        public void closureExample() {
            int factor = 2; // 这是一个局部变量
            
            // Lambda 表达式捕获了外部的 factor
            IntUnaryOperator multiplier = (number) -> number * factor;
            
            System.out.println(multiplier.applyAsInt(5)); // 输出 10
            
            // 如果取消下面一行的注释，编译器会报错，因为 factor 不再是 effectively final
            // factor = 3; 
        }
        ```
...
- 但可以有多个静态方法，多个默认方法和多个 `Object` 的 `#!java public` 方法
...
??? info "急性调用 vs 惰性调用"
    ...
    - **急性调用**(eager call)：在 Java 中，当调用形如 $y = f(x)$ 的函数时，$x$ 的值会在调用 $f()$ 之前就会被求解出来。特别地，当 $x$ 是一个调用另一个函数 $g()$ 的表达式时，即 $y = f(g(x))$ 时，$g(x)$ 应该要在调用 $f()$ 之前被执行和调用。这就是急切调用。
    - **惰性调用**(lazy call)：另一种方式是将整个函数调用 $g(x)$ 传递给 $f()$，然后让它在 $f()$ 的内部求解，因此叫做懒惰调用。此时 $f()$ 接收的是函数接口而不是求解好的参数值


### Stream

- **流**(stream)是实现了 `java.util.stream.Stream` 接口的对象，用于执行**函数式编程**任务
- 有专门针对处理 `#!java int`、`#!java long` 或 `#!java double` 值的流接口
- 流通过一系列处理步骤（称为**流管道**(stream pipeline)）移动元素
    - 管道从数据源开始，对数据源元素执行各种中间操作，并以终端操作结束
    - 流管道是通过**链式调用方法**形成的

- 生成流：可以从各种数据源创建 `Stream`，特别是 `Collections`, `List` 和 `Set`（⚠️注意不包括 `Map`）
    - `stream()`：为容器创建**串行流**
    - `parallelStream()`：为容器创建**并行流**（底层开启多线程，对数据进行分片）
...
- 中间操作：
    - 如果 `Stream` 只有中间操作是不会执行的；多个中间操作组成一个中间操作链，当执行终端操作的时候才会执行中间操作，这种方式称为**延迟加载**或**惰性求值**
...
- 终端操作：执行中间操作链，并返回结果
    ...
    - `#!java Optional<T> findFirst()`：返回当前流中的第一个元素
...
**RTTI**（运行时类型识别(run-time type identification)）：程序能在运行时发现和使用在编译时未知的类型

- **向上转型**与**向下转型**：最常见的 RTTI 形式，**多态**本身就是一种隐式的 RTTI
- `#!java instanceof` 关键字：用来判断一个对象是否属于某个特定类或其子类
- `Class` 对象：每个类在加载后都会在 JVM 中拥有一个对应的 `Class` 对象，它包含了类的元数据

**反射**(reflection)：程序在运行时观察和修改自身结构和行为的过程，涉及到以下任务：
...
>注：wk 讲过，但 lwm 没讲过，而且 wk 的作业中也没出现过这块内容，应该不会考。

`Optional` 类本质上是一个容器对象，它可以包含一个非空的值，也可以什么都不包含。引入它的核心目的是为了更优雅地处理可能为 `#!java null` 的情况，从而避免 Java 程序员最头疼的 `NullPointerException`，同时让代码的语义更加清晰。

对象创建：

- `Optional.of(value)`：仅适用于非常确定值不为 `n#!java ull` 的情况；若传入 `#!java null` 会立即抛出 `NullPointerException`
- `Optional.ofNullable(value)`：**最常用**，兼容性强，传入 `#!java null` 会得到一个空的 `Optional`，非 `#!java null` 则正常包裹
- `Optional.empty()`：直接生成一个空的 `Optional` 实例
...
- 取值
    - `get()`：（**慎用**）若容器为空会抛异常，这失去了 `Optional` 的意义
    - `orElse(defaultVal)`：为空时返回默认值
    - `orElseGet(() -> defaultVal)`：懒加载模式，只有为空时才执行 Supplier 生成默认值，性能更优
    - `orElseThrow()`：为空时抛出自定义异常

- 转换
    - `map(function)`：对值进行处理，结果自动封装为 `Optional`
    - `flatMap(function)`：防止嵌套；如果转换函数本身返回 `Optional`，用它可以展平结构
    - `filter(predicate)`：过滤值；若不满足条件，容器变为空

- 消费
    - `ifPresent(consumer)`：如果值存在，就执行一段代码，否则什么都不做；替代了传统的 `#!java if (obj != null)` 检查

最佳实践：

- 设计原则：当一个函数无法保证一定能返回结果时，其返回类型应定义为 `Optional`
- 优势：强制调用方在编译期就意识到空值的可能性，并编写相应的处理逻辑