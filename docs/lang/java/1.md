...
Java 还具备稳定性，并遵循 **KISS**(keep it simple, stupid) 原则。这体现在以下几方面：

- 没有指针
    - 但 Java 有类似指针的东西，叫做**引用**(reference)，不过它无法像指针那样直接参与计算，也不需要用到星号 `*`
    - 访问对象时只能通过引用去访问，并且一旦指定某个对象后就不得修改
- （数组）索引检查
    - 提高安全性，但是增加了额外的运行时间
- 自动内存管理
- 形式上和 C++ 类似
...
- Java 没有一个具体程序的概念
- Java 将所有的类和对象放在同一块空间；所以 Java 的 main 函数一定是属于某个类的，好处是可为每个类都可以创建 main 函数，然后可以自己指定执行哪个 main 函数；坏处是为了写一个 main 函数，不得不写一个完整的类
...
- 但对 Java 而言，所有的对象在运行时要构建好，并且要被存储在堆中
    - 所以必须自己创建对象，但对象不能被我们手动销毁——**垃圾回收**机制会帮我们清除不再使用的对象
...
- `#!java in.next()`：读入一个单词，读到空白字符为止（空格、Tab 键和换行）
    - 空白字符仍然留在缓冲区内，所以若空白字符是换行符，`next` 后紧接着的 `nextLine` 可能就返回一个空字符串
...
- 对象不能自己保存自己，需要用一个**句柄**(handle)（或**引用**）指代它
...
- 类名：每个 Java 程序至少有一个类名，一般以大写字母开头
- 主方法：要运行一个类，类里面就必须包含一个叫做 `main` 的方法，程序将从该方法执行
- 语句：表示单个行动或行动序列
- 语句终止符(statement terminator)：每条语句以分号（`;`）结尾
- 保留字(reversed word)（关键字）：对编译器而言有特殊含义的名称，在程序中不能用作其他目的
- 注释
- 块：程序中的一对花括号形成一个块，用于分组程序组件


## Programming Style and Documentation

- 合适的注释：在程序开始时对程序要做什么、关键特征、数据结构、独特技术等做一个总结
- 命名约定：有意义且有描述性的名称
- 正确的缩进（两个空格）和空行（分隔代码段）
- 块风格：花括号采用行末(end-of-line)风格（即左花括号不要另起一行）