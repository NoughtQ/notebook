...
- 方法：
    - `oridinal()`：返回枚举值在枚举类中的顺序，这个顺序根据枚举值**声明的顺序**而定，从 0 开始
...
- 子类继承父类（又称超类）用到的关键字是 `#!java extends`
...
- 父类的**构造函数不会被子类继承**(inherited)，子类需要显式/隐式调用父类的构造函数
    - **显式调用**时要用 `#!java super` 关键字，不用的话会报语法错误
    - 若没有显式使用该关键字，则会**自动调用父类的无参构造函数**（没有的话会触发编译错误）
    - 父类的构造函数总是会被调用：构造函数可以调用重载的构造函数或其父类的构造函数；如果它们都没有被显式调用，编译器会将 `super()` 作为构造函数中的第一个语句
...
**泛型编程**(generic programming)：多态允许方法对广泛的对象参数进行通用使用。
...
- 定义在类内部，可以实现对外部完全隐藏，可以有更好的封装性
- 内部类只是 Java 编译器的概念，而 JVM 不知道内部类这回事，因为每个内部类都会编译为一个独立的类，生成一个独立的 `class` 文件
- 内部类可以方便访问外部类的私有变量，可以声明为 `#!java private`，从而实现对外完全隐藏
- 调用内部类方法：
...
- 一个类可以有多个内部类：
    - 只要使用另一个内部类的对象作为调用对象，内部类可以访问彼此的私有成员
...
- 4 种内部类：
    - **静态内部类**(static inner class)
        ...
        - 可以**访问外部类的静态变量和方法**（无论 `#!java private` 还是 `#!java public`），但不可访问外部类的成员变量和方法
        - 外部类可以直接访问内部类
    ...
    - **成员内部类**
        - 没有 `#!java static` 修饰符
        - 外部类也可以直接使用内部类，而内部类也可以访问外部类的所有成员变量和方法（此时不区分 `#!java public` 和 `#!private`）
        - 成员内部类对象**需要与一个外部类对象绑定**
            ...
            // 错误做法
            java Outer.Inner inner = new Outer.Inner();
            ```
    ...
    - **方法内部类**(local class)
        - 定义在方法中，只能在定义的方法内用
        - 如果方法是**实例方法**, 除了静态变量和方法，内部类还可以直接访问外部类的实例变量和方法
        - 如果方法是**静态方法**，则内部类只能访问外部类的静态变量和静态方法
        - 可以访问方法的参数和局部变量，但必须是 `#!java final` 的
...
- **抽象类**(abstract class)是相对于**具体类**(concrete class)而言的，是一种**不能直接实例化**的类，因此不能使用 `#!java new` 运算符创建对象
...
- 一个类可以实现多个接口
    ...
    - 在少数情况下，一个类可能会实现的多个接口之间存在冲突信息（例如，两个相同的常量但值不同，或者两个具有相同签名但返回类型不同的方法），编译器会检测到这类错误
...
- Java 9 允许这两类方法是 `#!java private` 的（原来只允许 `#!java public`）
