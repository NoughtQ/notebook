...
---
此外，还有一种意义更明确的**成员函数模板**(member function templates)。它提供了一种泛型操作，让函数接受所有兼容类型，
避免了函数重载（而且有时候我们无法确定要重载多少函数）。
...
??? example "例子（以 `#!cpp std::share_ptr` 的实现为例）"
    ...
    MySmartPointer<Derived>: Default/Raw pointer constructor
    MySmartPointer<Derived>: Normal Copy Constructor
    Derived object

    --- Test 2: Same-type assignment ---
    MySmartPointer<Derived>: Default/Raw pointer constructor
    MySmartPointer<Derived>: Normal Copy Assignment Operator
    Derived object

    --- Test 3: Cross-type copy (Derived to Base) ---
    MySmartPointer<Base>: Template Conversion Constructor from MySmartPointer<Derived>
    Derived object

    --- Test 4: Cross-type assignment (Derived to Base) ---
    MySmartPointer<Base>: Default/Raw pointer constructor
    MySmartPointer<Base>: Template Conversion Assignment Operator from MySmartPointer<Derived>
    Derived object

    --- End of main ---
    MySmartPointer<Base>: Destructor
    MySmartPointer<Base>: Destructor
    MySmartPointer<Derived>: Destructor
    MySmartPointer<Derived>: Destructor
    MySmartPointer<Derived>: Destructor
    Base destructor
    Derived destructor
