...
>别看这里字数很多，其实有一大半内容都在讲 Advanced Topics...
...
- **代码复用**(code reuse)：为多种数据类型编写通用的算法和数据结构，从而避免重复考虑每种类型不同的具体实现。
- **泛型编程**(generic programming)：创建不依赖于任何特定类型的组件。根据传递给模板的类型参数，在编译时确定它们的行为。
- **隐式接口**(implicit interfaces)：不同于一般类以函数签名为中心的显式接口，模板采用的是基于**有效表达式**（或者说模板内执行的任何操作）的隐式接口。如果传入的类型不支持这些有效表达式，就无法通过编译。
- **编译时多态**(compile-time polymorphism)：与基于**虚函数**的**运行时多态**（在程序运行时解析调用）不同，**模板**在编译阶段就根据具体的类型参数生成特定的代码。这样通常能带来更高的性能，因为函数调用和类型信息在编译时就已经确定，从而减少运行时开销。
...
- **模板参数**：模板的灵活性来源于其参数。这些参数在模板定义时是未指定的占位符，在模板被使用（实例化）时才被赋予具体的值或类型。主要有以下几种模板参数：
    - **类型参数**(type parameters)：
        ...
        - 声明时使用的关键字 `#!cpp class` 或 `#!cpp typename`（例如 `#!cpp template <class T>` 或 `#!cpp template <typename T>`）几乎是**等价**的。
            - 但 `#!cpp typename` 还可用于消除**依赖名称**(dependence names)（依赖于模板参数的名称）的歧义。因为 C++ 默认把 `T::somename` 视为**非类型**，所以当引用一个嵌套在模板参数 `T` 内部的名称时，编译器不知道这指的是一个类型还是非类型成员。此时使用 `#!cpp typename` 表明这就是一个**类型**。

        - `T` 在模板定义中就代表了一个待定的类型，在实例化时会被替换为像 `#!cpp int`、`#!cpp std::string` 或我们自定义的类这样的具体类型。
...
---
此外，还有一种意义更明确的**成员函数模板**(member function templates)。它提供了一种泛型操作，让函数接受所有兼容类型，避免了函数重载（而且有时候我们无法确定要重载多少函数）。
...
??? example "例子"
    ...
    private:
        std::vector<int> data;
    public:
        // 成员函数模板：允许添加任何可以转换为int的类型
        template <typename T>
        void addElement(T value) {
            data.push_back(static_cast<int>(value)); // 尝试转换
    ...
    int main() {
        ...
        // mc.addElement("hello"); // 编译错误：string 无法转换为 int，体现了类型安全
        return 0;
    }
    ```

成员函数模板还能用于泛化拷贝构造和赋值操作，但此时也要保留一份正常的拷贝构造函数和拷贝赋值运算符的定义。

??? example "例子（以 `#!cpp std::shared_ptr` 的实现为例）"
    ...
    class Base {
    public:
        virtual void print() { std::cout << "Base object" << std::endl; }
        virtual ~Base() { std::cout << "Base destructor" << std::endl; }
    };

    class Derived : public Base {
    public:
        void print() override { std::cout << "Derived object" << std::endl; }
        ~Derived() { std::cout << "Derived destructor" << std::endl; }
    };

    // 模拟一个简化版的智能指针，展示其构造和赋值行为
    template <typename T>
    class MySmartPointer {
    private:
        T* m_ptr;
        // 实际的智能指针会有引用计数等资源管理机制

    public:
        // 1. 默认构造函数
        MySmartPointer(T* ptr = nullptr) : m_ptr(ptr) {
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Default/Raw pointer constructor" << std::endl;
        }

        // 2. 正常的拷贝构造函数 (同类型拷贝)
        MySmartPointer(const MySmartPointer& other) : m_ptr(other.m_ptr) {
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Normal Copy Constructor" << std::endl;
            // 实际智能指针会增加引用计数
        }

        // 3. 正常的拷贝赋值运算符 (同类型赋值)
        MySmartPointer& operator=(const MySmartPointer& other) {
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Normal Copy Assignment Operator" << std::endl;
            if (this != &other) {
                // 实际智能指针会释放旧资源，增加新资源的引用计数
                m_ptr = other.m_ptr;
            }
            return *this;
        }

        // 4. 模板转换构造函数 (不同类型拷贝，例如 Derived -> Base)
        template <typename U>
        // 确保 U* 可以安全转换为 T* (例如，U是T的派生类)
        // std::enable_if_t<std::is_convertible_v<U*, T*>, int> = 0 是 C++11/14 的 SFINAE 技巧
        // C++20 可以用 requires 关键字
        MySmartPointer(const MySmartPointer<U>& other)
            : m_ptr(static_cast<T*>(other.get())) { // 向上转型
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Template Conversion Constructor from MySmartPointer<" << typeid(U).name() << ">" << std::endl;
            // 实际智能指针会增加引用计数
        }

        // 5. 模板转换赋值运算符 (不同类型赋值)
        template <typename U>
        MySmartPointer& operator=(const MySmartPointer<U>& other) {
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Template Conversion Assignment Operator from MySmartPointer<" << typeid(U).name() << ">" << std::endl;
            // 注意：这里为了简化，没有做自赋值检查，实际需要更复杂的逻辑
            // 实际智能指针会释放旧资源，增加新资源的引用计数
        ...
        // 析构函数 (简化，实际智能指针会管理引用计数和删除)
        ~MySmartPointer() {
            std::cout << "MySmartPointer<" << typeid(T).name() << ">: Destructor" << std::endl;
            // 实际智能指针会在引用计数归零时 delete m_ptr;
        }
    };

    int main() {
        std::cout << "--- Test 1: Same-type copy ---" << std::endl;
        MySmartPointer<Derived> sp_d1(new Derived()); // Default/Raw pointer constructor
        MySmartPointer<Derived> sp_d2 = sp_d1;        // Calls Normal Copy Constructor
        sp_d2.get()->print();

        std::cout << "\n--- Test 2: Same-type assignment ---" << std::endl;
        MySmartPointer<Derived> sp_d3;                // Default constructor (nullptr)
        sp_d3 = sp_d1;                                // Calls Normal Copy Assignment Operator
        sp_d3.get()->print();

        std::cout << "\n--- Test 3: Cross-type copy (Derived to Base) ---" << std::endl;
        MySmartPointer<Base> sp_b1 = sp_d1;           // Calls Template Conversion Constructor
        sp_b1.get()->print();

        std::cout << "\n--- Test 4: Cross-type assignment (Derived to Base) ---" << std::endl;
        MySmartPointer<Base> sp_b2;                   // Default constructor (nullptr)
        sp_b2 = sp_d1;                                // Calls Template Conversion Assignment Operator
        sp_b2.get()->print();

        std::cout << "\n--- End of main ---" << std::endl;
        // 注意：这里为了简化，没有实现引用计数，所以会有内存泄漏和重复删除的风险
        // 实际的智能指针会正确管理内存
        return 0;
    }
    ```
    ...
    ```
    --- Test 1: Same-type copy ---
    MySmartPointer<7Derived>: Default/Raw pointer constructor
    MySmartPointer<7Derived>: Normal Copy Constructor
    Derived object

    --- Test 2: Same-type assignment ---
    MySmartPointer<7Derived>: Default/Raw pointer constructor
    MySmartPointer<7Derived>: Normal Copy Assignment Operator
    Derived object

    --- Test 3: Cross-type copy (Derived to Base) ---
    MySmartPointer<4Base>: Template Conversion Constructor from MySmartPointer<Derived>
    Derived object

    --- Test 4: Cross-type assignment (Derived to Base) ---
    MySmartPointer<4Base>: Default/Raw pointer constructor
    MySmartPointer<4Base>: Template Conversion Assignment Operator from MySmartPointer<Derived>
    Derived object

    --- End of main ---
    MySmartPointer<4Base>: Destructor
    MySmartPointer<4Base>: Destructor
    MySmartPointer<7Derived>: Destructor
    MySmartPointer<7Derived>: Destructor
    MySmartPointer<7Derived>: Destructor
    ```
...
- **模板类继承自类模板**：在已有的泛型类基础上，通过继承来扩展或特化其功能，同时保持泛型特性
    ...
    - 这种情况下，编译器在查找继承而来的名称时会遇到一个特殊的问题，即**依赖名称查找**(dependent name lookup)。但好在我们有以下方法可以明确告诉编译器，它应该在基类中查找这些继承而来的名称：
        - 使用 `#!cpp this->`（推荐）：因为 `#!cpp this` 指针的类型是依赖于模板参数的，因此任何通过 `#!cpp this->` 访问的成员都会被视为依赖名称。
        - 使用 `#!cpp using` 声明：将基类中的成员引入到派生类的作用域中。一旦引入，这些成员在派生类中就变成了非依赖名称。
        - 使用基类名限定：明确告诉编译器这个成员来自哪里。

- **非模板类继承自类模板的特定实例（特化）**：当我们需要一个特定类型的、基于某个泛型框架的类时，可以这样做；这相当于用一个具体的类型“固化”了泛型基类
...
!!! warning "概念区分"
    ...
    这里介绍的接口不是像 Java、Go 等语言中的 `#!java interface` 关键字（对应 C++ 就是抽象基类和纯虚函数的使用），而是更广义的、在软件设计和架构层面的概念，具体是指模块、组件或库向外部暴露的、供其他代码（通常是用户或客户端代码）进行交互和使用的约定和规范（有些抽象，但这就是一种“抽象”）。
...
在设计这些接口的时候，最核心的要求是：**让接口容易被正确使用，不易被误用**。
...
- 不易误用：
    ...
    - 束缚对象值
    - 消除客户的资源管理责任
- 任何接口如果要求用户必须记住做什么的话，就是有着“不正确使用”的倾向，因为用户可能会忘记做什么。
...
在 C++ 中，**特性类**(traits class)是一种强大的**模板元编程**技术，用于**在编译时获取关于类型的信息**，并根据这些信息来定制泛型代码的行为。它允许你为不同的类型提供不同的实现细节，而无需在运行时进行类型检查。

一个特性类通常是一个类模板或结构体模板，它通过**模板特化**为不同的类型提供特定的信息或行为。这些信息通常以：

- 嵌套类型：例如 `value_type`, `pointer`, `reference`。
- 静态常量：例如 `has_member_foo`, `is_integral<T>::value`。
- 静态成员函数：提供类型相关的操作。
...
- **编译时多态**：允许我们在编译时根据类型特性选择不同的代码路径，而不是在运行时使用虚函数或 `#!cpp dynamic_cast`。这消除了运行时开销，提高了性能。
- **泛型编程**的灵活性和效率：在编写通用算法（如STL算法）时，可能需要针对不同类型的迭代器、容器或数据类型采取不同的优化策略。T特性类允许算法查询这些类型的能力（例如，迭代器是否支持随机访问），从而选择最高效的实现。
- **提供缺失的类型信息**：有些内置类型（如指针 `#!cpp int*`）或第三方库的类型可能没有提供你需要的特定信息（例如，`#!cpp int*` 的值类型是 `#!cpp int`）。特性类可以为这些类型“补充”这些信息。
- **代码复用和模块化**：将类型相关的逻辑封装在独立的特性类中，使得泛型算法本身更简洁，也更容易维护和扩展。
- **避免运行时类型检查**：无需使用 `#!cpp typeid` 或 `#!cpp dynamic_cast`，这些操作有运行时开销，且不能用于所有编译时决策。


??? example "一个经典的例子：`#!cpp std::iterator_traits`"

    这是C++标准库中最著名的特性类之一。它允许泛型算法（如 `#!cpp std::advance`, `#!cpp std::distance`）查询迭代器的特性，例如：

    - `value_type`: 迭代器指向的元素类型。
    - `difference_type`: 两个迭代器之间的距离类型。
    - `iterator_category`: 迭代器的类别（输入、输出、前向、双向、随机访问），这决定了迭代器支持哪些操作。
    ...
    // 模拟一个泛型函数，根据迭代器类型选择不同的实现
    template <typename Iterator>
    void print_distance_info(Iterator first, Iterator last) {
        // 使用 std::iterator_traits 获取迭代器类别
        using Category = typename std::iterator_traits<Iterator>::iterator_category;

        std::cout << "Iterator Category: ";
        if constexpr (std::is_same_v<Category, std::random_access_iterator_tag>) {
            std::cout << "Random Access Iterator (efficient distance calculation)" << std::endl;
            // 对于随机访问迭代器，可以直接用 operator- 计算距离
            std::cout << "Distance: " << (last - first) << std::endl;
        } else if constexpr (std::is_same_v<Category, std::bidirectional_iterator_tag>) {
            std::cout << "Bidirectional Iterator" << std::endl;
            // 对于双向迭代器，可以从两端遍历，但通常还是单向遍历
            std::cout << "Distance (using std::distance): " << std::distance(first, last) << std::endl;
        } else {
            std::cout << "Other Iterator (e.g., Forward, Input)" << std::endl;
            // 对于其他迭代器，只能通过逐个递增来计算距离
            std::cout << "Distance (using std::distance): " << std::distance(first, last) << std::endl;
        }
    }
    ...
    int main() {
        ...
        print_distance_info(vec.begin(), vec.end());

        std::cout << "\n--- List Iterators ---" << std::endl;
        print_distance_info(lst.begin(), lst.end());

        // 对于原始指针，std::iterator_traits 也有特化
        int arr[] = {100, 200, 300};
        std::cout << "\n--- Raw Pointer Iterators ---" << std::endl;
        print_distance_info(arr, arr + 3);
