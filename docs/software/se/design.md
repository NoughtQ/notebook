...
**领域工程**(domain engineering)的三大核心活动是

- **分析**(analysis)：梳理和建模领域知识
- **构建**(construction)：开发可复用的领域资产
- **推广**(dissemination)分析：将这些资产应用到具体项
...
- 软件组件**检索系统**（例如对象请求代理），使客户端应用程序能够从库服务器检索组件和服务
    - 库查询通常使用 3C 模型中的**概念**(concept)、**内容**(content)和**上下文**(context)要素来描述
