...
- **白盒测试**(white-box testing)：设计能够执行软件模块**内部逻辑**的测试用例
    ...
    - 原因：
        ...
        - **拼写**(typographical)**错误**是随机的；未经测试的路径很可能会包含一些此类错误
    ...
    - **基本路径测试**(basis path testing)
        ...
        - **圈复杂度**(cyclomatic complexity) V(G) = 区域（**独立路径**）数 / E-N+2 / 谓词节点数 + 1；V(G)越高，错误概率就越高
            - 也可以用伪代码（PDL）表示
            - 注意：如果图中存在**复合条件**(compound condition)（比如 `if A && B`），独立路径数还要加一个
...
- **黑盒测试**(black-box testing)：设计能够证明每个程序**功能**都能正常运行的测试用例，能识别**不正确或缺失的功能**、**接口错误**、**性能问题**等
    ...
    - **等价类划分**(equivalence partitioning)：将输入域划分为数据类，从这些数据类中可以派生测试用例，以减少必须开发的测试用例总数
        - **等价类**(equivalence class)表示输入条件的一组**有效或无效**状态，因此没有特别的理由选择其中一个元素作为类代表而非另一个
        - 分类讨论：
            - 如果输入条件指定了一个**范围**，则定义一个有效等价类和两个无效等价类（超出下边界和上边界）
            - 如果输入条件要求一个特定**值**，则定义一个有效等价类和两个无效等价类（小于/大于该值）
    ...
    ??? example "例子"
        ...
        === "题目"
            ...
            Suppose a piece of code is used to calculate the real value of the function $f(x)=\sqrt{x-400}+\sqrt{1000-x}$ defined for some integers. Please design the test cases by applying equivalence partitioning and boundary value analysis technique.
...
- **划分测试**(partition testing)：
    - 以与传统软件等价类划分相同的方式，减少了测试一个类所需的测试用例数量
