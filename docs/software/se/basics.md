...
软件不会磨损(wear out)，但是会**退化**(deteriorate)。因为软件在使用过程中是会发生变化的，每次引入变化，失效率都会突然升高。
...
- **WebApps**（网络应用程序）
    - 现代 WebApps 远不止是带有少量图片的**超文本文件**
    - 通过 XML 和 Java 等工具得到增强，使网络工程师能够包含**交互式计算能力**（~~有些古典了~~）
    - 可以为最终用户提供独立功能，或与企业数据库及业务应用程序集成
    - **语义网技术**（Web 3.0）已发展成为复杂的企业及消费者应用程序，涵盖需要网络链接、灵活数据表示以及 API 进行访问的语义数据库
    - **内容的美学特性**仍然是决定 WebApp 质量的重要因素

- **移动应用程序**(mobile applications)
    - 包含既考虑设备特性又考虑位置属性的 UI
    - 通常提供对基于 Web 的资源与本地设备处理和存储能力的组合访问
    - 在平台内提供持久存储能力
    - 移动 Web 应用允许移动设备通过专为兼顾移动平台优缺点而设计的浏览器访问基于 Web 的内容
    - 移动应用可以直接访问设备硬件，提供本地处理和存储能力
    - 随着时间的推移，这些差异将变得模糊
...
- 级别0：不完整（过程未执行，或未实现该级别定义的所有目标）
- 级别1：已执行（正在执行生产所需工作产品所需的工作任务）
- 级别2：已管理（执行工作的人员拥有完成工作所需的充足资源，利益相关者积极参与，工作任务和产品受到监控、审查和评估，以确保符合过程描述）
- 级别3：已定义（管理和工程过程已文档化、标准化，并整合到组织级软件过程中）
- 级别4：量化管理（通过详细度量对软件过程和产品进行量化理解与控制）
- 级别5：优化（通过过程的量化反馈和创新理念的试验，实现持续的过程改进）
...
- **规定性模型**(prescriptive models)：主张采用**有序的方法**实现软件工程
    - **瀑布模型**(waterfall model)：
        - 遵循前面提到的经典的过程框架（5步骤）
        - 适用情况：在**需求已确定**的情况下，且工作采用线性方式完成的时候
    ...
    - **增量模型**(incremental model)
        - 适用情况：可能迫切需要用户迅速提供一套功能有限的软件产品，然后在后续版本中再进行细化和扩展功能
    ...
    - **演进模型**(evolutional model)
        - **原型开发**(prototyping)
            - 和瀑布模型的区别是需要在沟通后**迅速策划**一个原型开发迭代并进行建模 -> 快速设计 -> **原型**(prototype)
            - 在原型系统不断调整以满足各种利益相关者需求的过程中，采用迭代技术，同时也使开发者逐步清楚用户的需求
            - 适用情况：当**客户**有合理需求但对细节一无所知时，或者**开发人员**可能对算法的效率、操作系统的适用性和人机交互的形式等情况并没有把握
...
- **专用模型**(specialized model)
    - **基于组件的开发**(component based development)：复用是开发目标时所应用的流程
    - **形式化方法**(formal methods)：强调需求的数学化规范
...
- **个人模型**(personal model)（个人软件过程(**PSP**)）
    - 主要活动：
        ...
        2. **高层设计**(high-level design)：在正式编码之前，需要对软件结构进行初步设计，例如模块划分、接口设计等
...
- **会议非常简短**，有时在没有椅子的情况下进行；四种会议：
    - 冲刺计划会议(sprint planning meeting)：每个成员简要汇报前一天的工作进展、今天的工作计划和遇到的问题
    - **每日站会**(daily scrum)：每个成员简要汇报前一天的工作进展、今天的工作计划和遇到的问题
...
- 角色**定义不明确**，导致缺乏问责(accountability)，进而相互指责(finger-pointing)
...
- **开放范式**(opened paradigm)：试图以一种既能实现封闭范式中的某些管控，又能保留随机范式所带来的大量创新的方式来构建团队  
...
5. **做笔记并记录决策**：参与沟通的某人应担任“记录员”，记下所有重要观点和决策
...
3. 认识到**规划是迭代的**：项目计划绝不是一成不变的；随着工作的开展，情况很可能发生变化
