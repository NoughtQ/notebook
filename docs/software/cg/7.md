...
为方便讨论，下面以二维平面上的 AABB 为例讲解具体的计算过程，三维空间同理。核心思想是计算光线到达每个对面的最小时间和最大时间（$t_{\min}, t_{\max}$，可以是负数），并取中间的时间间隔（如下图红色线段所示），最后求个交集（右图）就是光线与包围盒相交的地方了。
...
对应的公式为：$t_{\text{enter}} = \max \{t_{\min}\}, t_{\text{exit}} = \min \{t_{\max}\}$。当 $t_{\text{enter}} < t_{\text{exit}}$，我们认为光线在包围盒内经过一会儿，所以它们必定会相交。然而光线不是（无方向的）直线，所以还需检查 $t$ 是否为正，否则这样的解是无效的。
...
**均匀空间划分**(uniform spatial partition)（网格(grid)）的步骤如下：
...
???+ abstract "均匀网格的成与败"
    ...
    - 何时失效：“茶壶在体育馆内”的问题，即物体在场景中的分布不均匀，某些地方集中出现很多物体，另一些地方却是大规模的空白（比如下图左上方的拱廊）
...
- 启发式方法2：按**中数**(median)物体所处位置（假如有 n 个物体，沿某个方向数量第 n/2 个物体）分割
    ...
    - 扩展：从 n 个无序的数找出第 i 大的数，可以在 O(n) 时间内完成——**快速选择算法**（类似快排，但递归时只考虑一边）
...
- 能够准确测量光的**空间**性质（不考虑时间维度）
    - 相关术语：**辐射通量**(radiant flux)、**辐射强度**(radiant intensity)、**辐照度**(irradiance)、**辐射率**(radiance)
- **以物理上正确的方式**执行光照计算
...
Whitted 风格的光线追踪是错误的；但刚才提到的**渲染方程**符合物理规律（辐射度量学），虽然是正确的，但计算太过复杂，因为
