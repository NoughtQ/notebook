...
另一种组合简单几何体的方法是使用**距离函数**(distance function)逐渐混合两个几何体的表面。距离函数能提供从任意位置到物体的最小距离（可以是**符号**距离，一般定义表面外一点到表面的距离为正，表面内一点到表面的距离为负）。
...
???+ example "例子"
    ...
    - 对于连通性那块的每一行，每个数据分别表示“顶点/纹理坐标/法线”（从 1 开始的索引），每行表示三个点的关系
...
??? play "动画演示"
    ...
    <div style="text-align: center">
        <img src="images/lec6/55.gif" width=60%>
    </div>
