...
- **边**(edge)：若沿着边的方向移动窗口，窗口内的像素没有发生变化
- **角**(corner)：无论向何种方向移动窗口，窗口内的像素都会发生明显的变化

下面将定量分析窗口内的像素变化：假定窗口$W$移动了$(u, v)$，我们通过计算窗口内每个像素变化的平方差(squared differences, SSD)之和来衡量窗口所有像素的改变量，记作误差$E(u, v)$，公式为：
...
我们可以用一个椭圆来表示矩阵$H$，用它的特征值表示两条轴的长度，用矩阵$R$表示椭圆的方向

<div style="text-align: center">
    <img src="images/C7/7.png" width="50%">
</div>

最后总结一下如何用这一工具寻找图像的特征点：

- 计算图像上每个像素点的梯度值
- 根据梯度得到矩阵$H$，并计算其特征值
- 找到最小特征值较大的像素点（满足$\lambda_- > \text{Threshold}$），这些点是局部最大值，可作为图像的特征点
...
---
$\lambda_-$是用于特征检测的哈里斯算子的变体。

下面给出**哈里斯角检测器**(Harris corner detector)或**哈里斯算子**(Harris operator)的公式：
...
- $f$的值与$\lambda_-$非常接近，但是前者的计算成本更低（没有平方根），下图比较了这两者的效果：

<div style="text-align: center">
    <img src="images/C7/10.png" width="50%">
</div>
...
- 对图像的加法和数乘强度改变具备部分的不变性
    - 强度移动：$I \rightarrow I + b$
...
可以看到，虽然这两幅图的缩放比例不同，但是左图最大的区域在图像中的比例和右图区域在图像中的比例看起来是一样的。

考虑一个问题：如何在图像上独立地选取不同像素点对应的区域？
...
- 设计一个关于区域（圆圈）的**缩放不变**(scale invariant)的函数，
...
- 共同的方法：取函数的局部最大值，因为它可以作为图像缩放过程中的不变量
- 需要注意的是每个图像的缩放不变量区域大小是相互独立的
- 一个好的函数应该只有一个峰值，一般来说好的函数与图像的**对比度**(contrast)相关
...
- 以下两种算法可以筛去前面两类不好的函数：
    ...
    - 这两种算法的思路大致相同：$f = \text{Kernel} \times \text{Image}$，且它们的核在缩放和旋转过程中保持不变
...
- 基于拉普拉斯算子$\nabla^2 g = \dfrac{\partial^2 g}{\partial x^2} + \dfrac{\partial^2 g}{\partial y^2}$，进行尺度的选择
    ...
    - 对于图像的每个像素点，我们将能够产生拉普拉斯值峰值的缩放比例称为**特征比例**(characteristic scale)
...
假设我们比较图像$I_1$和$I_2$，且$I_2$是$I_1$变换版本。无论它们之间发生何种变换，我们希望找到这两幅图相同的特征点，这称为**变换不变性**(transformational invariance)。有很多的特征方法可以处理平移、二维旋转、缩放等变换的不变性，甚至可以处理局部的三维旋转、仿射变换、亮度/对比度改变等。

**缩放不变特征变换**(scale invariant feature transform, SIFT)
...
**随机抽样一致**(RANdom SAmple Consensus)算法：
