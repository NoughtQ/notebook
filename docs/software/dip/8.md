...
??? abstract "核心知识"
    ...
    >PPT真的乱，所以我的笔记也是一坨，打算之后找其他资料补一下
...
神经元：对突触的观测值连到汇总的地方，并进行加权求和，通过激活函数产生输出
...
不同的神经元（**激活函数**(activation function)）：
...
1. 随机初始化权重，计算$h_w(X)$
2. 计算误差$E = (h_w(X) - y)^2$
3. 利用递推式$W_k = W_{k-1} - \varepsilon \dfrac{\partial E}{\partial w}$求解，具体来说，$w_{ij}^{(k)} = w_{ij}^{(k - 1)} - \varepsilon \dfrac{\partial E}{\partial w_{ij}^{(k - 1)}}$（梯度下降法）
...
深度学习的挑战（以一幅$200 \times 200$的图像为例）：

- 全连接$N \times N$，$40000$个内部单元，从输入到第1层的参数共有$40000 \times 40000$个
- 局部连接，$40000$个内部单元，在$10 \times 10$的区域内也要有$40000 \times 10 \times 10$个参数
- 局部连接捕获局部依赖

解决方法：**共享权重与卷积**

- 在图像某个部分有用的特征可能在其他地方也有用（稀疏表征）
- 移动等价处理（空间性质），即输入移动时，输出随之移动
- 卷积
    - 用到一个训练好的核/[**滤波器**](5.md)：$A_{ij} = \sum_{kl} W_{kl} X_{i + j, k + l}$
    - 过滤好的“图像”被称为特征图(feature map)
...
视觉的早分层特征模型(early hierarchical feature model for vision)

- 简单的单元用于检测局部特征
- 复杂的单元在视网膜定位的(retinotopic)邻居范围内“**池化**(pool)”简单单元的输出

<div style="text-align: center">
    <img src="images/C8/23.png" width="50%">
</div>

### Pooling

**池化**(pooling)是CNN中用于对特征图进行降维和信息提取的操作，通过减少数据量，提高了模型的计算效率，并保留主要特征，同时增强了特征的不变性（包括平移、旋转、尺度等）。
