...
!!! info "引入"
    ...
    在上篇笔记中，我们介绍了左偏堆和斜堆，虽然它们的平均插入速度为$O(\log N)$，已经是比较快的速度了，但是我们知道：往空堆插入$N$个节点的总体时间复杂度为$O(N)$，分摊到每个节点的时间复杂度就是$O(1)$，这说明插入速度还可以更快！为了追求极致的插入速度，我们引入了二项堆这一数据结构。
...
- `Merge`
    ...
    - 本质上是一个二进制加法：
        - 从最小的二项树开始合并（从低位到高位）
        - 合并相同规模的，且根节点值相对较小的二项树（同一位上的两个数相加）
...
- `DeleteMin`
    - 流程：
        1. 在二项堆中执行`FindMin`操作（找到最小值）（$O(\log N)$）
        2. 假设在$B_k$中找到最小值，那么将$B_k$从二项堆$H$中移除，剩下的二项堆记作$H'$（$O(1)$）
        3. 将$B_k$的根节点从$B_k$中移除，留下二项树$B_0, \dots, B_{k-1}$，记作$H''$（$O(\log N)$）
        4. 合并$H'$和$H''$，即`Merge(H', H'')`（$O(\log N)$）
    - 时间复杂度：$O(\log N)$
...
|操作|性质|解决方案|
|:-:|:---|:-----|
|`DeleteMin`|快速找到所有的子树|使用**Left-child-next-sibling**（链表）方法维护二项树|
...
???+ proof "证明"
    ...
    === "法2：势能分析法"
        ...
        解释：这样的插入操作会生成一棵二项树$B_{c-1}$，而其他二项树$B_i(0 \le i < c - 1)$会被移除。
...
!!! note "总结：各种堆的操作时间复杂度（by wyy）"
    ...
    |`Merge`|$O(\log n)$|$O(\log n)$|$O(\log n)$|$O(\log n)$|$O(1)$|
