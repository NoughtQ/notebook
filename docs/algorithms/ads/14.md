...
结论：根据排行问题给出的解，合并问题可以在$O(1)$时间内得到结果，且工作量为$O(n + m)$。所以该算法的速度非常快，但是工作量没有得到优化。
...
- 将数组$B$划分为多个大小为$n^{\frac{1}{8}}$的小块，因此有$n^{\frac{3}{4}}$个这样的小块。然后对每个小块求出最大值，通过两两比较的方式得到了$n^{\frac{3}{4}}$局部最大值
