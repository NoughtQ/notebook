...
- 这一算法与梯度下降法最大的不同之处在于：如果新的顶点覆盖$S'$的成本更大，它不会马上就会被抛弃掉，而是通过某个特殊的概率$e^{-\frac{\Delta \text{cost}}{kT}}$来决定它是否可以被保留下来（代码高亮部分）
...
- 局部最优解有多好？
    - 先给个结论：令$(A, B)$为一种局部最优的划分，$(A^*, B^*)$为一种全局最优的划分，那么$w(A, B) \ge \dfrac{1}{2}w(A^*, B^*)$。换句话说，<u>最大割的权重和至少是所有边权重和的一半</u>。下面给出证明：
    ...
    - 相关结论：
        ...
        - 该算法能够在至多$O(\dfrac{n}{\varepsilon} \log W)$次翻转后终止，其中$W = \sum w$
            ...
            ??? proof "证明"
                ...
                - 我们的目标是最大化$w(A, B)$，即令$w(A, B) \rightarrow W$，那么由上面的不等式知，要使$w(A, B)$从最低值1除法提升$W$倍，就需要翻转$O(\dfrac{n}{\varepsilon} \log W)$。得证。
...
- 若没有一个任务的时长超过所有任务总时长的一半，则机器$B$的总时长$T_2$满足关系：$\dfrac{1}{2} \le T_2 \le 2T_1$
