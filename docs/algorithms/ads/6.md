...
由于篇幅限制，列出八皇后问题的所有可行解不太现实，因此下面将以四皇后问题为例介绍解法（八皇后问题的解法与之基本一致，只是规模不太一样罢了）：
...
- 构建一棵**博弈树**(game tree)
    ...
    - 节点的数字表示搜索的顺序，深度为$i$的节点表示第$i$行上的皇后
    - 不难发现，这棵博弈树已经排除了两个皇后同行同列的情况，因此之后只需检验是否在同一对角线即可
    - 事实上，博弈树只是用来解释回溯法的一个抽象概念，所以不需要在程序中创建一棵真实的树
    ...
    === "解答"
        ...
        ???+ play "动画演示"
            ...
            === "初始状态（完成第二步后）"
                ...
                - 第1个点和最后那个点已用红色高亮标出，表示它们目前（暂时）确定位置，
                - 距离集$D$中的数字10被红色箭头指着，表示它（暂时）从集合中“删除”，我们不会再次用到这个距离了
        ...
        ??? question "问题：为什么第3步不是找“下一个**最小的**距离”"
            ...
            相对于找最小的距离，找最大的距离所产生的可能性更少（或者解空间更小）（通常来说只有两种情况，分别在靠近两端的位置上），可以让我们更加快速地查找；而最小距离对应的点的位置往往是很难确定的，这样反而会加大查找的难度。
...
我们采用**最小最大策略**(minimax strategy)来解决井字棋的问题。

- 假设玩家双方分别是人类和计算机，并且假定人类倾向于最小化位置$P$的值，而计算机倾向于最大化$P$的值
- 我们使用一个**评估函数**(evaluation function)来量化一个位置的**价值**(goodness)
