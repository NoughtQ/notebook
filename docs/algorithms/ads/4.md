...
- 具有与一般的堆一致的**顺序**性质，即每个节点一定比它所有的孩子节点大/小
...
1. 在不改变左孩子的情况下，根据两个堆的右子树进行合并。我的理解是：
    ...
    - 比较两个待比较堆，将**根节点更小的待比较堆去掉右子树后**附加到目标堆的右子树（即最右侧），形成新的目标堆，该目标堆的最右侧是空的
...
!!! warning "重要提醒"
    ...
    这里可不是说只要交换一次就好了——只要发现堆中存在右子树比左子树规模更大的情况，就应该进行（局部的）调整，因此有可能需要交换多次。
...
- 将刚刚提出来的子树分为3部分：节点$K$、左子树和右子树，合并左右子树（$O(\log N)$），形成新的子树$S$
- 将$S$插入回原来堆内节点$K$的位置，不断调整该子树祖先的$Npl$值，直到遇到某个不需要改变$Npl$值的节点为止
    - 何时需要调整$Npl$值：（待补充）
- 将节点$K$作为根节点插入到这个堆内，它的$Npl$值为0
...
!!! note "注"
    ...
    - 在最坏情况下，由于右侧路径很长，因此递归实现可能因为栈空间不足而无法实现
...
由于插入和删除本质上还是合并操作，所以这里我们就讨论合并操作的[均摊复杂度](1.md#amortized-analysis)，结论是：$T_{\mathrm{amortized}} = O(\log N)$，下面我们就来证明这个复杂度。

!!! proof "证明"

    均摊分析的三种方法中，势能分析法最为强大，因此我们使用这个方法。势能分析法中最重要的便是确定**势能函数**：我们令第$i$次操作后的数据集$D_i$为目标堆的根节点，势能函数$\Phi(D_i)$为这个堆上**重节点**(heavy node)的个数。
