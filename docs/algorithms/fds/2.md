...
此部分知识（大$O$、大$\Omega$、大$\Theta$表示法及其相关规则）在离散数学的3.2节中讲得更为详细，请移步[此处](../dm/3.md#the-growth-of-functions)。但这里还介绍了小$o$表示法：
...
+ [线性齐次递推关系](../dm/8.md#solving-linear-recurrence-relations)
...
采用**在线(on-line)算法**，它能在<u>任何时间点</u>上计算包含已经读入的数据的解
...
>参考：离散数学[相应章节](../dm/4.md#the-euclidean-algorithm)
