...
称 $[\mu - \varepsilon, \mu + \varepsilon]$ 是**置信区间**(confidence interval)，$\varepsilon$ 是**置信半径**(confidence radius)。
...
??? proof "证明"
    ...
    <div style="text-align: center">
        <img src="images/lec5/8.png" width="90%">
    </div>
...
接下来希望找到一个“好的”在线学习算法，想法仍然是定义遗憾，但此时遗憾的定义因为对手不是随机抽取的成本，因此需要调整。
...
???+ proof "分析"
    ...
    综合有 $e^{-T \varepsilon^2} \cdot e^{-\varepsilon \sum_{t=1}^T c_t(i)} \le ne^{-\varepsilon \sum_{t=1}^T \overline{C_t}}$，这等价于：
