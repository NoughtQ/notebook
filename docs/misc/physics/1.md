...
- 球形电容器：两同心金属球壳半径分别为$R_A, R_B$，电荷分别为$+q, -q$
    - 公式（用高斯定理 + 电势定义式推导）：$C = \dfrac{Q}{U_A - U_B} = 4 \pi \varepsilon_0 \dfrac{R_BR_A}{(R_B - R_A)}$
...
- **电位移**$\bm{D} = \varepsilon_0 \bm{E} + \bm{P} = (1 + \chi_e) \varepsilon_0 \bm{E} = \varepsilon_r \varepsilon_0 \bm{E} = \varepsilon \bm{E}$（单位：$\text{C/m}^2$），其中$\varepsilon$为介电常数。
